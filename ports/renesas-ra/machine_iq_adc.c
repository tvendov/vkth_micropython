...
static mp_obj_t machine_iqadc_inject(size_t n_args, const mp_obj_t *args) {
    ...
    if (point_arg < RA_IQ_INJECT_POINT_IN || point_arg > RA_IQ_INJECT_POINT_OUT) {
        mp_raise_ValueError(MP_ERROR_TEXT("invalid inject point"));
    }
    if (wave_arg < RA_IQ_INJECT_WAVE_SINE || wave_arg > RA_IQ_INJECT_WAVE_PULSE) {
