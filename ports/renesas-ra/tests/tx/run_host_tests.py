...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="ra-tx-core-") as temporary:
        binary = Path(temporary) / ("test_tx_core.exe" if os.name == "nt" else "test_tx_core")
        command = [
            compiler,
            "-std=c11",
            "-O2",
            ...
            str(Path(__file__).with_name("test_tx_core.c")),
            "-lm",
            "-o",
            str(binary),
        ]
        print("Compiling actual ra_tx_core.c with:", compiler, flush=True)
