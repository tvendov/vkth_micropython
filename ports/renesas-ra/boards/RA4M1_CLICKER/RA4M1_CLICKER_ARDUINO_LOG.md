...
2. Добавени са дефиниции **само** за част от ICU IRQ каналите, за да се съобразим
   с хардуерния лимит от 32 вектора при RA4M1, като последните два вектора са
   запазени за компаратора ACMPLP:

    - Нови вектори (индекси в `g_vector_table`):
      - `27` → `ICU_IRQ0`     (EXTINT0)
      - `28` → `ICU_IRQ1`     (EXTINT1)
      - `29` → `ICU_IRQ2`     (EXTINT2)
      - `30` → `ACMPLP0_INT`  (Comparator 0 interrupt)
      - `31` → `ACMPLP1_INT`  (Comparator 1 interrupt)
...
3. Съществуващите ICU вектори са запазени:
   ...
   Така общо имаме 6 активни външни прекъсвания: IRQ0..IRQ2, IRQ5, IRQ6 и IRQ9.
   Каналите IRQ3 и IRQ4 са освободени, за да се използват от компаратора
   (ACMPLP0/1) и вече **нямат** EXTINT поддръжка в тази конфигурация.
...
също и предвид ограничения брой активни канали (IRQ0..IRQ2, IRQ5, IRQ6 и IRQ9),
следните пинове на RA4M1 CLICKER могат **реално** да се ползват за външни
прекъсвания в текущата конфигурация:
...
Останалите хардуерни канали (IRQ3, IRQ4, IRQ7, IRQ8, IRQ10, IRQ11, IRQ12,
IRQ14, IRQ15)
...
## 6. Comparator (ACMPLP) и OPAMP – Arduino ↔ MicroPython примери

**Документация / примери в този порт:**

- `boards/RA4M1_CLICKER/RA4M1_CLICKER_COMPARATOR_OPAMP_EXAMPLES.md`

Там са събрани:

- MicroPython API за `machine.Comparator` (ACMPLP) и `machine.OPAMP`;
- прости примери за polling и IRQ върху компаратора;
- пример за стартиране на OPAMP канал;
- паралели с Arduino UNO R4 / Nano R4 (какво прави Arduino core-ът и какво
  прави MicroPython портът);
- стъпкови примери **Arduino C++ → MicroPython** – аналогово сравнение с
  `analogRead()` vs хардуерен Comparator, и концептуален пример за OPAMP.

Идея: този файл служи като „cheat sheet“ когато пренасяме проекти от Arduino
UNO R4 / Nano R4 към MicroPython върху RA4M1 CLICKER.
