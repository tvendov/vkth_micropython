...
Тази книга е направена върху реално наличния пакет от `65` примерни `.py` файла за `VK_RA4M2`.
...
- Глава 40. NeoPixel
- Глава 41. WS2812 през `machine.WS2812` и `SCI2`
...
- `WS2812 през специализиран C драйвер` -> `examples/ws2812_sci.py`, `examples/ws2812_sci_test.py`
- `WS2812 на P112 без външен clock pin към модула` -> обяснено в главата за `machine.WS2812`
...
Този драйвер кодира данните през `SCI2` и извежда само data сигнала на `P112`.

Практически това означава:

- към самия светодиоден модул свързвате само `DIN` към `P112`
- не подавате `SCK` и `MISO` към лентата
- продължавате да работите с буфер и `write()`, почти както при `NeoPixel`

Тук е важно да се мисли на две нива.

От гледна точка на Python кодът е еднопроводен: има само data pin към модула.

От гледна точка на микроконтролера вътрешно се използва `SCI2`, така че този път споделя хардуера си с `UART(2)` и `SPI(2)`.

Затова не трябва едновременно да очаквате:
...
from machine import WS2812  # Импортираме специализирания драйвер за WS2812 през SCI2.

vcc = Pin("P500", Pin.OUT, value=1)  # Подаваме малко захранване към единичния външен модул.
strip = WS2812(pixel_count=1, pin=Pin("P112"), channels=3)  # Създаваме драйверен обект върху P112.
strip[0] = (40, 0, 0)  # Зареждаме червено в буфера на първия пиксел.
strip.write()  # Изпращаме буфера към модула през SCI2 TX-only backend-а.
```
...
- data линията е фиксирана и не изисква да мислите за clock pin към самия модул
...
#### Резюме
- Главата показва `WS2812` като специализиран `machine` клас, реализиран в този порт върху `SCI2`.
- За самото свързване към модула е нужен само `P112` като data линия, без външен clock pin към светодиода.
- `machine.WS2812`, `UART(2)` и `SPI(2)` не могат да се използват едновременно, защото делят един и същ `SCI2` блок.

#### Самопроверка
1. Каква е разликата между `neopixel.NeoPixel` и `machine.WS2812` в този порт?
2. Защо потребителят свързва само `DIN -> P112`, въпреки че вътрешно драйверът използва `SCI2`?
3. Защо `UART(2)` и `WS2812` не бива да се използват едновременно?

#### Упражнения
1. Променете `pixel_count` на `2` или `4` и направете цветен „марш“ по къса лента.
2. Напишете функция `all_off()`, която извиква `fill((0, 0, 0))` и после `write()`.
3. Свържете `SW1` и сменяйте цвета на WS2812 модула при всяко натискане, без да използвате `UART(2)`.

#### Ако не работи
- Симптом: няма никаква светлина. Проверка: вижте дали `P500` реално подава захранване и дали `DIN` е към правилния вход на модула.
- Симптом: получавате грешка за зает bus или няма реакция след `UART(2)`. Проверка: `SCI2` може вече да е зает от `UART(2)` или `SPI(2)`.
- Симптом: цветовете са нестабилни при повече пиксели. Проверка: минете към отделно външно захранване и обща маса.
- Симптом: старият `neopixel` пример работи, а този не. Проверка: това е индикация, че точно `SCI2/P112` path-ът трябва да се валидира на реален хардуер.

#### Нататък
След тази глава прегледайте каталога на примерите и изберете дали да останете с универсалния `neopixel` подход, или с порт-специфичния `machine.WS2812` за `P112`.
...
- WS2812 path-ът през `machine.WS2812` и `SCI2/P112`, защото е нов порт-специфичен backend и трябва да се потвърди на реален хардуер
...
- всички `65` примерни файла са споменати изрично
