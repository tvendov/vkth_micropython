...
- `P500` като малко захранване за единичен модул
- `P112` като `DIN` data линия
- `1` пиксел за най-простия тест

Това е удобно за демонстрация, но има важно ограничение:

- за единичен модул или много кратка лента може да е достатъчно
- за по-дълга лента трябва отделно външно захранване и обща маса
...
```python
from machine import Pin  # Импортираме Pin за power и data линията.
import neopixel  # Импортираме neopixel драйвера за RGB модула.

vcc = Pin("P500", Pin.OUT, value=1)  # Подаваме захранване към малкия външен модул.
...
В учебните примери стойностите са ниски, например `40`, за да се ограничи токът и да се намали рискът при захранване на единичен модул от GPIO.
...
- ако се ползва външно захранване, масата трябва да е обща с борда
...
3. Защо при по-дълга LED лента не е добра идея да разчитате на `P500` като захранване?
...
- Симптом: единичният модул работи, а лентата не. Проверка: при повече пиксели минете към отделно външно захранване.
...
```python
from machine import Pin  # Импортираме Pin за малкото захранване и за data линията.
from machine import WS2812  # Импортираме специализирания драйвер за WS2812 с runtime pin избор.

vcc = Pin("P500", Pin.OUT, value=1)  # Подаваме малко захранване към единичния външен модул.
...
- логиката за кодиране и latch остава в C, а не в Python
...
- Симптом: цветовете са нестабилни при повече пиксели. Проверка: минете към отделно външно захранване и обща маса.
