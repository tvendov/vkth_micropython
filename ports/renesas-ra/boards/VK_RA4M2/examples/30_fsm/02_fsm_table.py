# Пример: Таблична FSM в MicroPython на VK_RA4M2.
...
print("=== Таблична FSM ===")  # Печатаме заглавие на примера.

for event_name in event_sequence:  # Обработваме събитията едно по едно.
    next_state = transition_table.get((state, event_name), state)  # Търсим новото състояние в таблицата или оставаме в старото.
    print("Събитие", event_name, ":", state, "->", next_state)  # Печатаме прехода в четим вид.
    state = next_state  # Обновяваме текущото състояние.

print("Примерът за таблична FSM приключи.")  # Завършваме примера.
