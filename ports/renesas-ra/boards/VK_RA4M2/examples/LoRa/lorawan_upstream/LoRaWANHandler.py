...
BUSY  = "P002"
RESET = "P001"
RF_SW = "P100"            # PE4529 /CTRL — drive HIGH винаги
...
class LoRaWANHandler:
    ...
    MaxEIRP = 14            # EU868 ETSI ERP лимит (не 16 dBm от Pi Pico variant)
...
#            print("RXcb receive: ", utime.ticks_ms())
    ...
    def __init__(self, configuration):
        ...
        # VK_RA4M2: enable RF switch (PE4529 /CTRL HIGH) преди SX1262 init.
        from machine import Pin as _Pin
        self._rf_sw_pin = _Pin(RF_SW, _Pin.OUT, value=1)

        print("Initializing SX1262 radio module")
        # Пълен ctor signature за micropySX126X — нашият sx126x.py е patched
        # вътрешно да ползва SoftSPI (firmware bug в hardware SPI(1)).
        self.SXRadio = SX1262(spi_bus=SPI_BUS, clk=CLK, mosi=MOSI, miso=MISO,
                              cs=CS, irq=DIO1, rst=RESET, gpio=BUSY)
        ...
        self.SXRadio.begin(
                ...
                tcxoVoltage=1.8, useRegulatorLDO=False, blocking=True)   # DCDC за +14 dBm PA

        # self.SXRadio.setBlockingCallback(False, TXcb)
        # self.SXRadio.setBlockingCallback(False, self.RXcb)
        # VK_RA4M2: блокира IRQ callback заради MemoryError при locked heap.
        # waitForRX() използва polling recv() с timeout — работи без callback.
    ...
    def loadDevNonce(self):
        try:
            ...
            print("Error reading DevNonce value! (start with 1)")
            self.DevNOnce = 1                      # TTN отхвърля DevNonce=0 като replay
...
#        self.currentFreq = 0
    ...
    def otaa(self):
        ...
        _freq = self.nextFreq()
        self.SXRadio.setFrequency(_freq)
        self.setDR(self.currentDR)
        print("=== JoinRequest TX diagnostic ===")
        print("  Freq:    %.3f MHz" % _freq)
        print("  DR:      %d (SF%d BW%d)" % (self.currentDR, self.DRList[self.currentDR][1], self.DRList[self.currentDR][0]))
        print("  Power:   %d dBm" % self.TXPowerTable[self.currentPower])
        print("  DevEUI:  ", bytes(self.DevEUI).hex())
        print("  JoinEUI: ", bytes(self.JoinEUI).hex())
        print("  AppKey:  ", bytes(self.AppKey).hex())
        print("  DevNonce:", currentDevNOnce.hex())
        ...
        msg = lorawan.to_raw()
        print("  Frame:   ", bytes(msg).hex(), "(%d bytes)" % len(msg))
