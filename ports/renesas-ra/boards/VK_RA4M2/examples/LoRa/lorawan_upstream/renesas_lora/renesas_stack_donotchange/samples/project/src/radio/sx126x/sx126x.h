...
typedef enum
{
    ...
    MODE_COLD_SLEEP                                         //! The radio is in cold sleep mode
...
typedef struct
{
    ...
    struct
    {
        ...
        {
            uint8_t RxStatus;
            int16_t RssiAvg;                                //!< The averaged RSSI
            int16_t RssiSync;                               //!< The RSSI measured on last packet
            uint32_t FreqError;
        }Gfsk;
        struct
        {
            int16_t RssiPkt;                                //!< The RSSI of the last packet
            int8_t SnrPkt;                                  //!< The SNR of the last packet
            int16_t SignalRssiPkt;
            uint32_t FreqError;
        }LoRa;
    }Params;
}PacketStatus_t;
...
/*
 * following structures are added by renesas
 */

/*!
 * \brief Declares the Fallback mode after tx/rx
 */
typedef enum
{
    FB_FS                                   = 0x40,
    FB_STDBY_XOSC                           = 0x30,
    FB_STDBY_RC                             = 0x20,
}RadioFallbackModes_t;

/*!
 * PIB Entity Definition
 */
typedef struct
{
    int8_t   rssiOffset;
...
/*!
 * \brief Recover TCXO Oscillation
 *
 */
void SX126xRecoverBoardConfig( void );

/*!
 * \brief Returns the offsetted RSSI value specifyed PIB PIB_RSSI_OFFSET
 *
 * \retval      rssi      RSSI value not offseted
 */
int16_t SX126xCalculateOffsettedRssi(int16_t rssi);

/*!
 * \brief Gets the RF frequency.
 *
 * \retval RF frequency. 0 means not set or need set.
 */
