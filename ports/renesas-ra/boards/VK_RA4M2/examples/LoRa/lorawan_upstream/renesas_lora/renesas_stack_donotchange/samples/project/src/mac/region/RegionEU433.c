...
/* duty cycle enabled	*/
uint8_t EU433_DutyCycleEnabled = EU433_DUTY_CYCLE_ENABLED;
...
PhyParam_t RegionEU433GetPhyParam( GetPhyParams_t* getPhy )
{
    ...
    switch( getPhy->Attribute )
    {
        ...
        {
            /* duty cycle enabled   */
            phyParam.Value = EU433_DutyCycleEnabled;
            break;
        }
...
void RegionEU433SetPhyParam( SetPhyParams_t* setPhy )
{
    switch( setPhy->Attribute )
    {
        case PHY_DUTY_CYCLE:
        {
            EU433_DutyCycleEnabled = setPhy->param.dcycle_enabled;
            break;
        }
        default:
        {
...
void RegionEU433InitDefaults( InitDefaultsParams_t* params )
{
    ...
    switch( params->Type )
    {
        case INIT_TYPE_INIT:
        {
            ...
            // DutyCycle setting
            EU433_DutyCycleEnabled = EU433_DUTY_CYCLE_ENABLED;

            // (output) default PhyParams
            params->DefaultDutyCycleOn       = EU433_DutyCycleEnabled;             // PHY_DUTY_CYCLE
            params->DefaultChannelsTxPower   = EU433_DEFAULT_TX_POWER;             // PHY_DEF_TX_POWER
            params->DefaultChannelsDatarate  = EU433_DEFAULT_DATARATE;             // PHY_DEF_TX_DR
            params->DefaultMaxRxWindow       = EU433_MAX_RX_WINDOW;                // PHY_MAX_RX_WINDOW
            params->DefaultReceiveDelay1     = EU433_RECEIVE_DELAY1;               // PHY_RECEIVE_DELAY1
            params->DefaultReceiveDelay2     = EU433_RECEIVE_DELAY2;               // PHY_RECEIVE_DELAY2
            params->DefaultJoinAcceptDelay1  = EU433_JOIN_ACCEPT_DELAY1;           // PHY_JOIN_ACCEPT_DELAY1
            params->DefaultJoinAcceptDelay2  = EU433_JOIN_ACCEPT_DELAY2;           // PHY_JOIN_ACCEPT_DELAY2
            params->DefaultRx1DrOffset       = EU433_DEFAULT_RX1_DR_OFFSET;        // PHY_DEF_DR1_OFFSET
            params->DefaultRx2Frequency      = EU433_RX_WND_2_FREQ;                // PHY_DEF_RX2_FREQUENCY
            params->DefaultRx2Dr             = EU433_RX_WND_2_DR;                  // PHY_DEF_RX2_DR
            params->DefaultUplinkDwellTime   = 0;                                  // PHY_DEF_UPLINK_DWELL_TIME
            params->DefaultDownlinkDwellTime = 0;                                  // PHY_DEF_DOWNLINK_DWELL_TIME
            params->DefaultMaxEirp           = EU433_DEFAULT_MAX_EIRP;             // PHY_DEF_MAX_EIRP
            params->DefaultAntennaGain       = EU433_DEFAULT_ANTENNA_GAIN;         // PHY_DEF_ANTENNA_GAIN
            params->DefaultAdrAckLimit       = EU433_ADR_ACK_LIMIT;                // PHY_DEF_ADR_ACK_LIMIT
            params->DefaultAdrAckDelay       = EU433_ADR_ACK_DELAY;                // PHY_DEF_ADR_ACK_DELAY
            break;
        }
...
void RegionEU433ApplyCFList( ApplyCFListParams_t* applyCFList )
{
    ...
    if( (applyCFList->Size != 16) || (applyCFList->Payload[15] != 0) )
    {
        ...
        for( uint8_t removeChId = EU433_NUMB_DEFAULT_CHANNELS; removeChId < EU433_MAX_NB_CHANNELS; removeChId++ )
        {
...
void RegionEU433ComputeRxWindowParameters( int8_t datarate, uint8_t minRxSymbols, uint32_t rxError, RxConfigParams_t *rxConfigParams )
{
    ...
    rxConfigParams->Datarate = R_MIN( datarate, EU433_RX_MAX_DATARATE );
...
bool RegionEU433ChannelsRemove( ChannelRemoveParams_t* channelRemove  )
{
    ...
    ret = RegionCommonChanDisable( NvmCtx.ChannelsMask, id, EU433_MAX_NB_CHANNELS );
    if( ret == true )
    {
