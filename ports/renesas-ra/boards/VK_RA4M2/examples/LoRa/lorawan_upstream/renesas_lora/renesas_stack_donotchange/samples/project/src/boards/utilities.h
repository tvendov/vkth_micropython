...
#define CRITICAL_SECTION_BEGIN( ) { uint32_t mask; BoardCriticalSectionBegin( &mask ); if( g_critical_sectioin_count++ == 0) { BoardTimerDisableIrq(); } BoardCriticalSectionEnd( &mask ); }

/*!
 * Ends critical section
 */
#define CRITICAL_SECTION_END( ) { uint32_t mask; BoardCriticalSectionBegin( &mask ); if(--g_critical_sectioin_count == 0) { BoardTimerEnableIrqNoClear(); } BoardCriticalSectionEnd( &mask ); }
