...
/* duty cycle enabled   */
uint8_t KR920_DutyCycleEnabled = KR920_DUTY_CYCLE_ENABLED;
...
PhyParam_t RegionKR920GetPhyParam( GetPhyParams_t* getPhy )
{
    ...
    switch( getPhy->Attribute )
    {
        ...
        case PHY_DUTY_CYCLE:
        {
            phyParam.Value = KR920_DutyCycleEnabled;
            break;
        }
...
void RegionKR920SetPhyParam( SetPhyParams_t* setPhy )
{
    switch( setPhy->Attribute )
    {
        case PHY_DUTY_CYCLE:
        {
            if( KR920_DutyCycleEnabled != setPhy->param.dcycle_enabled )
            {
                KR920_DutyCycleEnabled = setPhy->param.dcycle_enabled;
#if defined(RP_USE_RADIO_CFG_CHECK)
                Radio.SetPib( PIB_RADIO_CFG_CHECK_ENABLE, (uint8_t *)&KR920_DutyCycleEnabled );
#endif
            }
            break;
        }
...
void RegionKR920InitDefaults( InitDefaultsParams_t* params )
{
    ...
    switch( params->Type )
    {
        case INIT_TYPE_INIT:
            ...
            NvmCtx.RssiFreeThreshold = KR920_RSSI_FREE_TH;
            NvmCtx.CarrierSenseTime = KR920_CARRIER_SENSE_TIME;

            // DutyCycle setting
            KR920_DutyCycleEnabled = KR920_DUTY_CYCLE_ENABLED;

            // (output) default PhyParams
            params->DefaultDutyCycleOn       = KR920_DutyCycleEnabled;             // PHY_DUTY_CYCLE
            params->DefaultChannelsTxPower   = KR920_DEFAULT_TX_POWER;             // PHY_DEF_TX_POWER
            params->DefaultChannelsDatarate  = KR920_DEFAULT_DATARATE;             // PHY_DEF_TX_DR
            params->DefaultMaxRxWindow       = KR920_MAX_RX_WINDOW;                // PHY_MAX_RX_WINDOW
            params->DefaultReceiveDelay1     = KR920_RECEIVE_DELAY1;               // PHY_RECEIVE_DELAY1
            params->DefaultReceiveDelay2     = KR920_RECEIVE_DELAY2;               // PHY_RECEIVE_DELAY2
            params->DefaultJoinAcceptDelay1  = KR920_JOIN_ACCEPT_DELAY1;           // PHY_JOIN_ACCEPT_DELAY1
            params->DefaultJoinAcceptDelay2  = KR920_JOIN_ACCEPT_DELAY2;           // PHY_JOIN_ACCEPT_DELAY2
            params->DefaultRx1DrOffset       = KR920_DEFAULT_RX1_DR_OFFSET;        // PHY_DEF_DR1_OFFSET
            params->DefaultRx2Frequency      = KR920_RX_WND_2_FREQ;                // PHY_DEF_RX2_FREQUENCY
            params->DefaultRx2Dr             = KR920_RX_WND_2_DR;                  // PHY_DEF_RX2_DR
            params->DefaultUplinkDwellTime   = 0;                                  // PHY_DEF_UPLINK_DWELL_TIME
            params->DefaultDownlinkDwellTime = 0;                                  // PHY_DEF_DOWNLINK_DWELL_TIME
            params->DefaultMaxEirp           = KR920_DEFAULT_MAX_EIRP_HIGH;        // PHY_DEF_MAX_EIRP
            params->DefaultAntennaGain       = KR920_DEFAULT_ANTENNA_GAIN;         // PHY_DEF_ANTENNA_GAIN
            params->DefaultAdrAckLimit       = KR920_ADR_ACK_LIMIT;                // PHY_DEF_ADR_ACK_LIMIT
            params->DefaultAdrAckDelay       = KR920_ADR_ACK_DELAY;                // PHY_DEF_ADR_ACK_DELAY

            // Intentional fall through to set channels
            // no break;
...
void RegionKR920ApplyCFList( ApplyCFListParams_t* applyCFList )
{
    ...
    for( chanIdx = KR920_NUMB_DEFAULT_CHANNELS; chanIdx < KR920_MAX_NB_CHANNELS; chanIdx++ )
    {
    ...
    pos = 0;
    for( chanIdx = KR920_NUMB_DEFAULT_CHANNELS; chanIdx < ( KR920_NUMB_CHANNELS_CF_LIST + KR920_NUMB_DEFAULT_CHANNELS ); chanIdx++ )
    {
        // Channel frequency
...
void RegionKR920ComputeRxWindowParameters( int8_t datarate, uint8_t minRxSymbols, uint32_t rxError, RxConfigParams_t *rxConfigParams )
{
    ...
    rxConfigParams->Datarate = R_MIN( datarate, KR920_RX_MAX_DATARATE );
...
bool RegionKR920TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    maxEIRP = R_MIN( txConfig->MaxEirp, maxEIRP );
...
uint8_t RegionKR920LinkAdrReq( LinkAdrReqParams_t* linkAdrReq, int8_t* drOut, int8_t* txPowOut, uint8_t* nbRepOut, uint8_t* nbBytesParsed )
{
    ...
    while( bytesProcessed < linkAdrReq->PayloadSize )
    {
        ...
        if( linkAdrParams.ChMaskCtrl != 6 )
        {
            ...
            {
                for( i = 0; i < KR920_MAX_NB_CHANNELS; i++ )
                {
                    if( ( ( chMask & ( 1 << i ) ) != 0 ) &&
...
#if (KR920_TX_MIN_DATARATE > 0)  // Avoid warning; comparing uint8 value with 0 is meaningless
    if( channelAdd->NewChannel->DrRange.Fields.Min < KR920_TX_MIN_DATARATE )
    {
        drInvalid = true;
    }
#endif
    if( ( channelAdd->NewChannel->DrRange.Fields.Min > channelAdd->NewChannel->DrRange.Fields.Max ) ||
        ( channelAdd->NewChannel->DrRange.Fields.Max > KR920_TX_MAX_DATARATE ) )
    {
        drInvalid = true;
    }
...
bool RegionKR920ChannelsRemove( ChannelRemoveParams_t* channelRemove  )
{
    ...
    ret = RegionCommonChanDisable( NvmCtx.ChannelsMask, id, KR920_MAX_NB_CHANNELS );
    if( ret == true )
    {
...
void RegionKR920SetContinuousWave( ContinuousWaveParams_t* continuousWave )
{
    ...
    maxEIRP = R_MIN( continuousWave->MaxEirp, maxEIRP );
