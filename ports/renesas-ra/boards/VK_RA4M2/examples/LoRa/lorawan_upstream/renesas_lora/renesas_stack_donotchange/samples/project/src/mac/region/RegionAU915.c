...
/* duty cycle enabled   */
uint8_t AU915_DutyCycleEnabled = AU915_DUTY_CYCLE_ENABLED;
 ...
 */
static uint32_t RegionBaseUSCalcDownlinkFrequency( uint8_t channel, uint32_t frequency,
                                                   uint32_t stepwidth )
{
...
PhyParam_t RegionAU915GetPhyParam( GetPhyParams_t* getPhy )
{
    ...
    switch( getPhy->Attribute )
    {
        ...
        case PHY_DUTY_CYCLE:
        {
            phyParam.Value = AU915_DutyCycleEnabled;
            break;
        }
...
void RegionAU915SetPhyParam( SetPhyParams_t* setPhy )
{
    switch( setPhy->Attribute )
    {
        case PHY_DUTY_CYCLE:
        {
            if( AU915_DutyCycleEnabled != setPhy->param.dcycle_enabled )
            {
                AU915_DutyCycleEnabled = setPhy->param.dcycle_enabled;
#if defined(RP_USE_RADIO_CFG_CHECK)
                Radio.SetPib( PIB_RADIO_CFG_CHECK_ENABLE, (uint8_t *)&AU915_DutyCycleEnabled );
#endif
            }
            break;
        }
...
void RegionAU915InitDefaults( InitDefaultsParams_t* params )
{
    ...
    switch( params->Type )
    {
        case INIT_TYPE_INIT:
        {
            ...
            NvmCtx.JoinChannelGroupsCurrentIndex = 0;            // Channels
            ...
            // DutyCycle setting
            AU915_DutyCycleEnabled = AU915_DUTY_CYCLE_ENABLED;

            // (output) default PhyParams
            params->DefaultDutyCycleOn       = AU915_DutyCycleEnabled;             // PHY_DUTY_CYCLE
            params->DefaultChannelsTxPower   = AU915_DEFAULT_TX_POWER;             // PHY_DEF_TX_POWER
            params->DefaultChannelsDatarate  = AU915_DEFAULT_DATARATE;             // PHY_DEF_TX_DR
            params->DefaultMaxRxWindow       = AU915_MAX_RX_WINDOW;                // PHY_MAX_RX_WINDOW
            params->DefaultReceiveDelay1     = AU915_RECEIVE_DELAY1;               // PHY_RECEIVE_DELAY1
            params->DefaultReceiveDelay2     = AU915_RECEIVE_DELAY2;               // PHY_RECEIVE_DELAY2
            params->DefaultJoinAcceptDelay1  = AU915_JOIN_ACCEPT_DELAY1;           // PHY_JOIN_ACCEPT_DELAY1
            params->DefaultJoinAcceptDelay2  = AU915_JOIN_ACCEPT_DELAY2;           // PHY_JOIN_ACCEPT_DELAY2
            params->DefaultRx1DrOffset       = AU915_DEFAULT_RX1_DR_OFFSET;        // PHY_DEF_DR1_OFFSET
            params->DefaultRx2Frequency      = AU915_RX_WND_2_FREQ;                // PHY_DEF_RX2_FREQUENCY
            params->DefaultRx2Dr             = AU915_RX_WND_2_DR;                  // PHY_DEF_RX2_DR
            params->DefaultUplinkDwellTime   = AU915_DEFAULT_UPLINK_DWELL_TIME;    // PHY_DEF_UPLINK_DWELL_TIME
            params->DefaultDownlinkDwellTime = AU915_DEFAULT_DOWNLINK_DWELL_TIME;  // PHY_DEF_DOWNLINK_DWELL_TIME
            params->DefaultMaxEirp           = AU915_DEFAULT_MAX_EIRP;             // PHY_DEF_MAX_EIRP
            params->DefaultAntennaGain       = AU915_DEFAULT_ANTENNA_GAIN;         // PHY_DEF_ANTENNA_GAIN
            params->DefaultAdrAckLimit       = AU915_ADR_ACK_LIMIT;                // PHY_DEF_ADR_ACK_LIMIT
            params->DefaultAdrAckDelay       = AU915_ADR_ACK_DELAY;                // PHY_DEF_ADR_ACK_DELAY
            break;
        }
...
void RegionAU915ComputeRxWindowParameters( int8_t datarate, uint8_t minRxSymbols, uint32_t rxError, RxConfigParams_t *rxConfigParams )
{
    ...
    rxConfigParams->Datarate = R_MIN( datarate, AU915_RX_MAX_DATARATE );
...
LoRaMacStatus_t RegionAU915NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    if( nbEnabledChannels > 0 )
    {
        ...
        RegionCommonChanDisable( NvmCtx.ChannelsMaskRemaining, *channel, AU915_MAX_NB_CHANNELS );
