...
AtResultCode_t AppComplianceAtModeSet(void *p)
{
    ...
    if (argc == 1)
    {
        ...
        if (argvLen == 1)
        {
            ...
            {
                appLoraWanSettings.complianceTestMode = APP_COMPLIANCE_TESTMODE_FUOTA200;
                certFportOn = 1;
                ret = AT_RC_OK; 
            }
#endif
...
#if defined(FUOTA_ENABLED) && (FUOTA_VERSION >= FUOTA_VERSION_2_0_0)
    else if (complianceTestMode == APP_COMPLIANCE_TESTMODE_FUOTA200)
    {
        AppAtOutputResultCode("2:FUOTA_V200_COMPLIANCE_TEST_ENABLED");
    }
#endif
    else
    {
