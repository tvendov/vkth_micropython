...
void LoRaMacConfirmQueueSearch( Mlme_t request, MlmeConfirmQueue_t **pp_mlmeConfirm )
{
    MlmeConfirmQueue_t  *p_element;
    uint8_t             i;
    ...
    if( ConfirmQueueCtx.ConfirmQueueNvmCtx->MlmeConfirmQueueCnt > 0 )
    {
        p_element = ConfirmQueueCtx.BufferStart;

        for (i = 0; i < ConfirmQueueCtx.ConfirmQueueNvmCtx->MlmeConfirmQueueCnt; i++)
        {
            if (p_element->Request == request)
            {
                break;  // found. exit from for(i) loop
            }

            p_element = IncreaseBufferPointer( p_element );
        }
        if (i == ConfirmQueueCtx.ConfirmQueueNvmCtx->MlmeConfirmQueueCnt)
        {
            p_element = NULL;  // not found
        }
    }

    (*pp_mlmeConfirm) = p_element;
}
