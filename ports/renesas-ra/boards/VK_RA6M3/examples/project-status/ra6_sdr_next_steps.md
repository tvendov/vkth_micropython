...
## Ново изискване — 2026-09-09: антенен релеен изход

Предложено разпределение, още без активиране:

| Функция | GPIO | Посока | Статус |
| --- | --- | --- | --- |
| PTT | P114 | Вход | Предложен; очаква потвърждение за външните връзки |
| Морзов ключ | P115 | Вход | Предложен; очаква потвърждение за външните връзки |
| Антенно реле RX/TX | P107 (D7) | Изход към драйвер | Свободен в проверения текущ SDR код; физически непроверен |

Приоритет: включване на релето в общото AUTO/PTT/CW управление,
преди да се счита за безопасно автоматичното антенно превключване.
Това е управляващ GPIO, **не захранване за бобината**. Нужни са модел/
напрежение/ток на релето или готовия модул и неговото активно ниво.
Драйверът трябва да има определено неактивно ниво при reset и защита
за индуктивния товар. Не се задават произволни времена преди проверка
на operate/release/contact-bounce параметрите на конкретното реле.

Последователност за реализация и проверка:

1. Към TX: спряно излъчване → програмиране/подготовка → реле към TX →
   изчакване на установяване на контактите → разрешаване на TX сигнала.
2. Към RX: спиране/заглушаване на TX и проверка на резултата → реле към
   RX → изчакване на установяване → разрешаване на RX. Само I/Q нулеви
   кодове не доказват липса на RF от външен модулатор/усилвател.
3. В PTT режим релето следва действителното разрешение за предаване,
   не само факта, че е отворен TX екран. AUTO запазва директното TX
   поведение, но също преминава през релейната последователност.
4. В CW да не превключва механично за всяка точка/тире. Да се уточни
   задържане в TX или време за връщане; първият знак не бива да се губи
   по време на превключването. Морзовият ключ и PTT остават отделни входове.
5. При грешка/reset: спиране на TX, безопасно неактивно управление на
   драйвера. Ако спирането не е потвърдено, не се превключват контактите
   под RF товар; нужен е изричен fault/interlock договор с външния модул.
6. Host тестове за реда на операциите и отказите; HIL на GPIO; накрая
   проверка с реалното реле и RF тракт. Тестове само в RAM.

Статус: **планирано, не е имплементирано/качено**. Тази итерация не
докосва работещата платка. Evidence и ограниченията са в done.

## Ново изискване — 2026-09-09: физически PTT и морзов ключ

Потребителят поиска два отделни входа и избра **AUTO/PTT**, не задължителен
PTT за всички случаи. AUTO запазва директното предаване при избрано TX.
В PTT режим отпускането трябва да премахва модулирания I/Q изход;
морзовият ключ управлява CW с оформени фронтове, отделно от PTT.

Предложени: **PTT=P114, KEY=P115**, активни при контакт към GND, с pull-up.
`VK_RA6M3/pins.csv` ги свързва с D5/D6; не са конфигурирани в board
`ra_gen/pin_data.c` и не се използват от текущия SDR Python код.
Имат и алтернативни PWM функции — предложението не е универсално
твърдение, че никоя външна верига не ги използва. Изчаква се потвърждение
за реалното окабеляване; входовете **не са конфигурирани/качени**.

| Работа | Статус / приемане |
| --- | --- |
| Резервиране на P114/P115 | Предложение; без запис в PFS преди потвърждение |
| AUTO/PTT в контекстния TX BACKEND | За реализация; отделно TX запазване, AUTO по подразбиране |
| Native входно управление | За реализация; debounce, защитено key-up и mute при отпускане/грешка, без зависимост от LVGL кадрите |
| CW ключ | Наличен native `IQTX.key()`; няма физически вход. Не се вика тежкият DTC rearm без анализ от Pin IRQ |
| Показване на armed/PTT/key състоянието | За реализация; да не се представя безшумен armed режим като действително предаване |
| Тестове | Host логика, HIL open/ground/release, RX↔TX и soft-reset cleanup; без тестови скриптове във flash |

## Текущ приоритет — 2026-09-09: TX аудиолента и изходно ниво

Одобрено: избор на звукова лента AM/USB/LSB, действителен филтър за
MIC/R:xx/GEN, HOME/BACKEND контроли, отделни RX/TX настройки; добавено PWR.

| Задача | Статус | Следващо действие |
| --- | --- | --- |
| Native TX AF 100 Hz–избрана горна граница; BASE | Host и кратък HIL с profile/crossfade PASS | Дълъг stress; измереният C body max не е абсолютен ISR worst case |
| MIC / FILE / GEN към AM / USB / LSB | Всичките9 цифрови HIL комбинации PASS | Известен аналогов MIC вход и честотен отговор |
| HOME хоризонтален избор / TX BACKEND AF upper | Реални програмно изпратени LVGL callbacks PASS | Физически touch/layout; transition UND остава |
| PWR 0–100% | HIL DAC registers при0/40/100 PASS; owner не се сменя | Аналогов размах; не се твърдят ватове |
| RX/TX параметри | Отделни bw/txbw, VOL/txlevel; host roundtrip PASS | Dataflash readback след връщане RX |
| RadioOnly firmware + MPY | Качено на1120000058/COM25; readback PASS;76 стари файла запазени | Без тестови файлове във flash; PTT/KEY не са в този build |

AM профили: BASE/2k/3k/4.5k/6k; USB/LSB: BASE/1.8k/2.4k/2.7k/3k.
BASE пази предишния път; SSB и FILE anti-alias филтрите остават.
PWR е линеен цифров размах, не измерена RF мощност. TX save остава
отложен до RX; захранването не бива да се спира преди записването.
Няма нов framebuffer или per-sample Python обработка. Native filter state
308 B се побира в CW workspace; целият TX static owner нараства с 12 B
заради конфигурация/статус, но общият BSS и heap резерв не нарастват в този build.
Наблюдавани C body max: GEN AM1449/5000 цикли, GEN SSB4511/10000;
MIC AM1227/2728, MIC SSB4309/10000, FILE SSB4729/10000. Няма deadline
отказ в този тест; няма доказателство за абсолютен worst case.
UI heap тренд и допълнителният спрямо предишен build разход не са измерени.
R:USB натрупа85 underrun семпъла при преход; steady UND0 във всичките
MIC/FILE случаи. UI performance поправката остава отложена.

## Отложено — 2026-09-09: HF проверки и репитърна сигнализация
...
## Решение — 2026-09-09: отложени проверки; следваща разработка

По новото решение на потребителя отлагаме повторното изследване на
Si5351 readback и физическите AM/USB/LSB I/Q измервания. Те не са
завършени и не се възобновяват автоматично при „следваща стъпка“.
Репитърните функции и UI performance работата остават отложени.

Първоначалното предложение за TX филтър вече е одобрено и реализирано
в локалната версия по-горе. Това не възобновява отложените Si5351/RF
проверки. Последното потвърдено състояние на платката остава предходният
production лог; не е правено ново текущо измерване.
...
| Si5351 TX x1/RX LO x4 | Единичен readback FAIL в първия soak;6 диагностични и12 повторни цикъла PASS | НЕИЗЯСНЕНО; допълнителната проверка е отложена от потребителя |
...
| Аналогово AM/USB/LSB I/Q и външен модулатор | Непроверено; отложено от потребителя | Физическо измерване с два канала, после RF спектър |

Оставащи проверки; т.1 и т.2 са отложени, не са текуща следваща стъпка:
