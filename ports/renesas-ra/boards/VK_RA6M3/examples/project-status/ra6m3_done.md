...
## 2026-09-09: антенен релеен изход — source проверка и изискване

Потребителят поиска отделен изход за пренасочване на антената RX/TX.
Предложен е **P107 (D7)**; това не променя предложените P114/PTT и
P115/морзов ключ. Не е променяна конфигурация на GPIO или firmware.

Проверени източници в `ports/renesas-ra`:

- `boards/VK_RA6M3/pins.csv:147`: D7 е P107.
- `build-VK_RA6M3/pins_VK_RA6M3.c:75,355`: генерираният Pin обект и
  D7 alias сочат P107. Това доказва наличност в порта, не електрическо състояние.
- `boards/VK_RA6M3/ra_gen/pin_data.c`: няма начална конфигурация за P107.
- `boards/VK_RA6M3/mpconfigboard.h:88`: PWM назначението за P107 е коментирано.
- `boards/VK_RA6M3/examples/sdr_single.py:7240`: текущият дисплей е pRGB.
  `modules/Lvg.py:43-49` използва D7 за ST7789 подсветка само в RA6M5
  клона; RA6M3 клонът използва Pdrv. Това не е конфликт в текущия SDR.

Следователно P107 е свободен според проверената текуща конфигурация.
Външното окабеляване и електрическото състояние НЕ са проверени.
Релето не трябва да се свързва с бобината директно към GPIO; нужен е
подходящ драйвер и защита за индуктивния товар. Моделът, напрежението,
токът, активното ниво и времената за превключване още не са уточнени.

В текущия код `_start_tx_owner()` програмира синтезатора и стига до
`tx.start()` без управление на антенен релеен изход. `_release_tx_checked()`
спира и освобождава TX преди възстановяване на RX. Това са проверените
точки за бъдещото последователно управление, не вече реализирана функция.
Новото изискване и критериите са записани в `ra6_sdr_next_steps.md`.
Само source анализ и документация: без build, COM/J-Link, reset, flash или HIL.

## 2026-09-09: TX AF/PWR — качване и цифров GEN тест на платката

Качени са точно артефактите от `vk-ra6m3-tx-audio-band-drive-v1` на
J-Link1120000058 / COM25. RadioOnly BIN1596072 B, SHA256
`88f641d14e2c37dd40d4333ab8f3583009df552baca15d6f80100071cc57315c`;
runtime MPY94066 B, SHA256
`cd12449adfe5b28ad90716eade51d322fa0d9af5cb9583654b2bec050f46e70c`.
Editable source е `/flash/sdr_single.py.source`; приложението не е frozen.
OpenCV остава изключен; linker heap резервът е281600 B, не свободният UI heap.

Преди запис е направен архив
`backups/tx-filter-1120000058-20260909-035048/`: вътрешен flash2MiB,
външен QSPI16MiB, валидният dataflash record, файлов manifest и предишно
production състояние. Прочетеният стар BIN съвпадна с очаквания архив.
Новият BIN и двата application файла са прочетени обратно и съвпадат.
Всичките76 предишни файла са запазени; старото приложение е преименувано
в `/flash/sdr_single.pre-txfilter.mpy` и `.py.source`. Boot/main, IQ записите,
резервираните4MiB QSPI и валидният dataflash record не са променени при
deployment. Не са записвани тестови скриптове във flash.

GEN цифров HIL: AM/USB/LSB, всичките15 HOME филтърни избора и BACKEND
изборът PASS. Заявеният и активният native cutoff съвпадат, pending се
изчиства; TX owner остава същият. RX ширините и VOL остават непроменени.
PWR0/40/100 PASS: при0 двата DAC register кода са2048; I размахът при40%
е приблизително0.4 от този при100%. AM Q остава2048. Това са цифрови
регистрови extrema, не синхронни I/Q вектори или измерена RF мощност.

| GEN режим | Измерен C DSP max / budget, цикли | Deadline / DSP clips / filter clips |
| --- | --- | --- |
| AM | 1449 / 5000 | 0 / 0 / 0 |
| USB | 4509 / 10000 | 0 / 0 / 0 |
| LSB | 4511 / 10000 | 0 / 0 / 0 |

Max включва наблюдаваните смени и crossfade, но измерва C тялото,
не пълна ISR латентност или абсолютен worst case. AF кадрите и sample
броячите напредват. UI събитията са изпратени програмно към реалния LVGL;
физически touch и аналогов/RF тест не са правени. След всеки изолиран
тест има NORMAL J-Link reset.

MIC и FILE цифров HIL: всичките6 AM/USB/LSB × MIC/R:xx комбинации PASS.
Новият cutoff се активира, owner остава същият, post-filter AF кадрите и
DSP семплите напредват. При установена работа UND/deadline/clip са0.
При прехода към R:USB има85 file underrun семпъла; не се представя като
безгрешен преход. Съществуващото UI transition ограничение остава.

| Източник / режим | C DSP max / budget, цикли |
| --- | --- |
| MIC AM | 1227 / 2728 |
| R:AM | 1234 / 5000 |
| MIC USB / LSB | 4309 / 10000 |
| R:USB / R:LSB | 4729 / 10000 |

Първият MIC harness спря на неверен тестов assert: търсеше `_tx_file_label`
равно на `MIC`, докато правилното представяне е `None` за микрофона.
Поправен е само RAM harness; повторният6-случаен тест PASS. Първият
неуспех и последвалият J-Link reset са запазени в протокола.
Не е подаван известен аналогов микрофонен тестов сигнал и не е измерван
аналоговият честотен отговор. Стойностите не доказват RF готовност.

Накрая е стартирано каченото приложение и е възстановено прочетеното
преди deployment състояние: **TX AM579900 Hz, GEN SINE1000 Hz50%,
PWR40%, AM depth50%, AF BASE**. Native API version1, running/outputs
enabled; AF frames63, DSP samples61025, UND/CLIP/deadline0 в snapshot.
`gc.mem_free()` е57552 B в този момент, не гарантиран минимум.
Обичайният save handler е възстановен. COM25 е затворен; няма reset
след финалния production старт. Архивът съдържа изпълнените RAM payload-и
и HIL/production logs; Git протоколът е `tests/tx/results/tx-filter-board-20260909/`.

Добавено е изискването за отделни PTT/морзов входове и избран AUTO/PTT.
Предложени са P114/P115 след source проверка; реализацията и окабеляването
са отделни от качения AF/PWR build. Подробният статус е в next steps.
...
Няма промяна на производствения Python/C код, нов build или запис на
тестове във flash. J-Link1120000058 / COM25; RAM сценариите се подават
изцяло и се проверяват с echo/SHA преди стартиране на RX.

1. Контроли: AM/USB/LSB × MIC/съответния R:xx, временно на3.5 MHz,
   TX LEVEL40, gain100, AM depth50. Реални LVGL callbacks за SOURCE,
   BACKEND/BACK, LOOP OFF→ON, RX↔TX; не физическо натискане на панела.
   AF напредва; scope OFF спира capture, не DSP. LEVEL0/20/40 дава
   очакваните native амплитуди, на0 и двата DAC кода са2048; gain80→100
   и depth75→50 се прилагат без смяна на TX owner. AM Q=2048.
2. Всичките шест комбинации: CLIP0, deadline0, unexpected_callbacks0;
   без нов UND в измерените5 s след установяване. Но файловите източници
   вече са натрупали UND8130/6916/6816 при предходните действия. Това
   НЕ е непрекъснат FILE стрийм; не е измерено кой точно callback ги причинява.
3. Първият12-циклов soak FAIL-ва след lap0 AM/MIC. При lap1 USB/R:USB
   Si5351 CLK1 readback за3.5 MHz е `00 ff 00 f0 00 00 00 00`, вместо
   `ff ff 00 70 4d fd ff cd`. Отказът и точният скрипт са запазени;
   след него е направен reset. Не е установено дали е грешен запис,
   прочит или друг механизъм; физическата clock честота не е измерена.
4. Отделна RAM диагностика, без поправящи записи:6 RX↔TX цикъла,
   27 проследени записа и72 съвпадащи readback проверки. Допълнителните
   прочити/печат променят времето, затова не затварят първия отказ.
5. Повторен ОРИГИНАЛЕН soak, без тази диагностика:12 цикъла за193585 ms,
   всяка mode/source двойка по два пъти. BACKEND/BACK и keypad/CANCEL;
   във всеки установен10 s интервал: AF напредва, нов UND0, CLIP0,
   deadline0. FILE завърта записите6..7 пъти. RX free66480..66640 B
   след първите3 цикъла, с RAM теста в heap; не доказва липса на дълъг leak.
   Преди тези интервали FILE UND11247..14820 при групите действия остава.
   TX x1/RX LO x4 readback минава в повторението; първият отказ е НЕИЗЯСНЕН.

Проверени са производственият firmware readback1594536 B и SHA на
installed source/MPY — непроменени от предходната итерация. Firmware SHA:
`692fa2c29ed92d5a16d005c6f3669ba6a4ea18603f3731a572236b4be41181a3`.
В controls/clock/успешния повторен soak валидният368 B SDR1 dataflash
запис е идентичен преди/след. За неуспешния soak има само before снимка.
Няма пълен QSPI readback тази итерация; файловете не са презаписвани.

След всеки изолиран набор — J-Link NORMAL reset. Накрая е възстановен
точният `before.json`: TX AM579900 Hz, GEN SINE1000 Hz/50%, TX LEVEL40,
depth50, LOOP=True. AF напредва, CLIP/deadline/UND/error0 и CLK1 readback
съвпада. Нормалният settings-save callback е възстановен; няма reset
след този производствен старт. Приложението е оставено да работи.

Промени/доказателства по файлове:

- `tests/tx/results/hf-voice-20260909/controls-ram.py` и логовете:
  шестте комбинации и действителните UI/control assertions.
- `soak-ram.py`, `soak-partial.log`, `soak-error.txt`, `soak-host.log`:
  първият отказ, без замяна с успешния резултат.
- `clock-ram.py` и логовете: диагностичният прогон, не firmware поправка.
- `soak-repeat1-ram.py` и логовете: същият оригинален сценарий и12 цикъла.
- `production-ram.py`, `before.json` и логовете: действителното
  начално състояние, проверки на installed файлове и възстановяване.
- `host-hf-voice.py`, `host-production.py`: архив на RAM orchestration;
  използват съществуващите локални помощници и не са преносим installer.
- `README.md`, `verify_evidence.py`, `evidence-audit.log`: обхват,
  ограничения и offline PASS за6 controls, запазения FAIL,72 readbacks,
  12 цикъла,5 upload SHA и368 B настройки. При съставяне на audit-а са
  поправени разчитането на escaped error repr и Windows CRLF срещу
  uploaded LF; това са host проверки, не нови board откази.
- `ra6_sdr_next_steps.md`: действителен статус и критерии за останалото;
  двата канонични дневника и Git копията са синхронизирани.

Пълните първични файлове са в
`backups/hf-voice-1120000058-20260909-011714`.
MIC stimulus е непотвърден; няма физически touch, аналогово I/Q или RF
измерване. Следват изолиране на sporadic Si5351 readback и физическо
AM/USB/LSB приемане. Допустимите меню-преходи не се представят като UND0.

## 2026-09-09: смяна на приоритета към HF

Потребителят поиска да прескочим репитърната сигнализация.

- `ra6_sdr_next_steps.md`: CTCSS,1750 Hz, тон с глас и тоново заглушаване
  са отложени, не завършени. Следва HF AM/USB/LSB с MIC/R:xx, физическо
  I/Q приемане и продължителна проверка на стабилността.
- `tx_tone_signalling.md`: бъдещата репитърна част е обозначена като
  отложена; наличните GEN и пасивен TONE MON остават без промяна.
- Каноничните документи и Git работните им копия са синхронизирани.

Само промяна на документация/приоритет. Няма кодова промяна, build,
качване, reset или ново измерване; предходният HIL остава последното
проверено състояние, не нова текуща проверка на платката.

## 2026-09-09: плавни TX GEN преходи — firmware, качване и повторен HIL

Изпълнена е следващата записана задача след теста на GEN нивата.
Промяната е само в C генератора и тестовете; UI, модулатори, limiter,
Si5351 настройки и framebuffer подход не са променяни.

Промени по файлове:

- `ra/ra_tone.h`: отделно `ra_tone_smooth_t`28 B и init/request/reset/next
  API. Суровият DDS12 B и RX detector80 B остават непроменени.
- `ra/ra_tone.c`: целочислена обвивка; пълно 0→100% за20 ms, закръглени
  до следващ семпъл. Live ниво се движи от текущата стойност; при смяна
  на честота/форма първо затихва до0, сменя при нулев изход и се връща
  до новото ниво. До40 ms плюс два семпълни периода. Фазата продължава;
  повторна заявка не рестартира прехода. Делението е при init, не на семпъл.
- `ra/ra_tx_hw.c`: използва smooth състоянието (+16 B static), приема
  готовите настройки под съществуващата критична секция и подава точно
  същия AF към scope и модулатора. Live промени не чистят DSP историята.
  Явният start/нов режим започва нов поток с нулева фаза и плавно включване.
- `tests/tx/test_gen_transitions.c`: нови вектори за реалните C генератор
  и AM/FM/USB/LSB модулатори: граници, рационален Fs, фаза, повторни заявки,
  смяна на форма само при0, mute и точно съвпадение с установения DDS.
  `GEN_SMOOTH=0` е старият рязък вариант и умишлено FAIL-ва CLIP проверката.
- `tests/tx/run_host_tests.py`: включва новия набор с реалния tone C код.
- `tests/tx/test_tx_af.py`: поправена остаряла host постановка — втория
  DWT аргумент на `tx_cpu_sample` и helper за софтуерен източник. Добавени
  512 GEN семпъла: scope е точно същият AF като модулатора; scope OFF
  спира само наблюдението, не генератора. PASS3524 проверки.
- `tx_tone_signalling.md`: актуален договор за преходите, единици,
  памет, доказан обхват и оставащи ограничения. Двата дневника обновени.
- `tests/tx/results/gen-smooth-20260909/`: host/build/deploy/HIL протокол,
  точните RAM сценарии, запазени откази и проверка на комплектността.

Host резултати: core18 групи/8403968 проверки, tone и13 Python-driven
набора PASS. Старият GEN върху същите14 етапа/24 фазови отмествания:
AM0 / FM168 / USB1312 / LSB1312 CLIP; новият —0 за всички. Това сравнение
е host, не аналогово измерване. Отрицателна контрола SQUARE100%/1kHz:
новият вариант все още отчита FM1851 и USB/LSB1799 CLIP — реалното
претоварване НЕ се прикрива. FM mute residual0.018775 Hz е съществуващ
DC-servo ефект при dev2500/gain100, не ново обещание за целия диапазон.

Първият широк host run спря заради остарелия AF test stub; поправен е
тестът, не производственият callback. Предходният CLI отказ без `--cc`
също е host-only. И двата лога са запазени; не са причина за board reset.

Shared `build-VK_RA6M3`, RadioOnly, OpenCV=0, USER_C_MODULES празно,
`/mingw64/bin/python3.exe`, `-j16`: PASS. Остава linker warning за RWX
LOAD segment; тази итерация не го поправя. Размери:

| Показател | Преди | След | Разлика |
| --- | ---: | ---: | ---: |
| firmware.bin | 1594288 B | 1594536 B | +248 B |
| ELF BSS | 649896 B | 649912 B | +16 B |
| Heap по linker symbols | 281600 B | 281600 B | 0 B |

Нов ELF text1594549 B, data0. Firmware SHA256:
`692fa2c29ed92d5a16d005c6f3669ba6a4ea18603f3731a572236b4be41181a3`.
Build label `911b6c1f92-dirty` е преди commit-а на тази итерация.
След build е уточнен само коментарът за sample закръглянето и текстът
на host PASS съобщението; изпълнимото C поведение не е променяно.

Качване през J-Link1120000058; COM25 за RAM сценарии. Първо пълен backup,
после само вътрешен firmware и byte-for-byte readback. Всички16MiB QSPI
са идентични преди/след;76 файла, source/MPY и IQ записи не са презаписани.
Валидният368 B dataflash запис е запазен. Нестабилните прочити след
края му в512 B тестовите снимки не се броят като променени настройки.
Архив: `backups/gen-smooth-1120000058-20260909-004135`.

Цифров HIL след качване:

- 20 нива AM/USB/LSB/FM ×0/25/50/75/100%: CLIP=0 натрупано, преходно
  и след установяване. AM I размах0/102/204/306/408; USB/LSB
  0/408/818/1228/1636. AM Q=2048. Това са кодове, не измерени волтове.
- 36 комбинации: четири режима ×1000/1750/500 Hz ×SQUARE/TRIANGLE/SINE,
  при GEN50%, gain100, TX LEVEL40, depth50/dev2500 —0 CLIP.
- TX DSP max/budget цикли в теста на нивата: AM592/5000,
  USB/LSB4428/10000, FM1114/5000. Това са callback high-water стойности,
  не измерване на целия ISR и не точен сравнителен CPU benchmark.
- 8 RX↔TX цикъла за102.542 s: без TX CLIP/deadline/FILE UND и MemoryError;
  RX free66848..66960 B, GUI менюта/клавиатури и CLK1 TX x1/RX LO x4 PASS.
- RX native TONE MON: AM/USB/LSB/FM1kHz, CW700 Hz, FM100 Hz, грешен тон,
  тишина, BYP, OFF, stop/start PASS; установените mode интервали имат
  audio underruns/ring overruns=0. AM измерен DSP32% OFF→38% ON.
- Отделен малък VERIFY тест:1750 Hz TONE→1000 Hz WAIT→OFF→BACK PASS;
  HOME free75872 B, VERIFY7920 B, след BACK74624 B при зареден RAM тест.
- След всеки изолиран набор — J-Link NORMAL reset. Няма тестови файлове
  във flash. Целият RAM скрипт е echo/SHA проверен преди RX start.

Финално приложението е пуснато от непроменения installed source/MPY:
TX AM579900 Hz, GEN SINE1000 Hz/50%, TX LEVEL40, depth50, MIC gain100.
AF кадрите напредват, изходите са enabled, CLIP/deadline/error/FILE UND=0.
Възстановено нормалното записване на настройки; няма reset след този старт.

Не е измервана физическа I/Q фаза, напрежение, RF или touch. MIC/FILE
soak, UART загубите под RX и тон с реч остават отворени. GEN замества
микрофона; отделен тон, смесен с гласа за репитър, още няма.

## 2026-09-08: GEN нива и 16 RX↔TX цикъла — цифрова проверка

Тази итерация не променя приложението или firmware. Използван е същият
J-Link 1120000058 / COM25. Тестовете са подадени само в RAM, с проверка на
целия скрипт чрез SHA преди старта на RX. След всеки изолиран набор има
J-Link NORMAL reset; няма тестови файлове, записани във flash.

Промени по файлове и доказателства:

- `tx_gen_level_soak_20260908.py`: host сценарий за действителните GUI
  callbacks, 20 комбинации модулация/ниво и 16 RX↔TX цикъла. Разделя
  натрупаните, преходните и новите установени CLIP събития. Проверява
  вървящи AF/DSP броячи, GEN без ADC/FILE, Si5351 readback и освобождаване
  на клавиатурата. Host зависимостите са от работната тестова среда;
  пълните генерирани RAM сценарии са архивирани отделно.
- `check_gen_test_record_20260908.py`: диагностичен прочит на SDR1 записа
  след неправилното първоначално сравнение на всички 512 B dataflash.
- `start_gen_after_soak_20260908.py`: проверка на инсталираните образи и
  старт на производственото приложение с настройките, прочетени тази
  итерация; възстановява нормалния save callback. Няма ново качване.
- `tests/tx/results/gen-levels-soak-20260908/`: пълни RAM сценарии, host
  логове, snapshots на настройките и протокол с границите на проверката.
- `verify_evidence.py` в същия архив: offline проверка PASS на четирите
  RAM script SHA/синтаксис, двете 368-B сравнения, 20 нива и 16 цикъла.
  Това е проверка на съхранените доказателства, не повторен HIL.
- `tx_tone_signalling.md`: обяснени измерените GEN нива, разликата спрямо
  FM девиацията и преходните CLIP събития. Този done и next са синхронизирани
  с Git копията в `examples/project-status`.

Условия: GEN SINE 1000 Hz; GEN LEVEL 0/25/50/75/100%; TX LEVEL 40;
AF gain 100%; AM depth 50%; FM deviation 2500 Hz; RF 579900 Hz.

| Режим | Размах на I в DAC кодове при GEN 0/25/50/75/100% | Q |
| --- | --- | --- |
| AM | 0 / 102 / 204 / 306 / 408 | Постоянен код 2048 |
| USB | 0 / 408 / 818 / 1228 / 1636 | Променлив |
| LSB | 0 / 408 / 818 / 1228 / 1636 | Променлив |

За всяка точка има 1 s изчакване, следвано от 192 независими прочита.
Във всички 20 измервателни интервала новите CLIP са 0; DSP deadline misses
и FILE underruns също са 0. Фазата между I/Q и напреженията НЕ са измерени:
`ra_tx_hw_get_status()` чете DADR0/DADR1 неатомарно. При FM амплитудата на
аудиото управлява девиацията, не пропорционално I/Q обвивката.

Остава ограничение при резки промени: USB/LSB стартират на 100% след
предишния режим с по 10 CLIP; преходът към 0% добавя по 10. При 75→100%
USB добавя 7, LSB 6. FM започва с 5 натрупани CLIP, без нови в следващите
измервателни интервали. В source CLIP означава ограничаване след аудио
обработката (`ra_tx_core_ssb_sample` / `ra_tx_core_fm_sample`), не непременно
DAC код 0/4095. Плавен преход още не е реализиран. FM `adc_rails` на GEN
100% брои цифровия входен код 4095; при GEN няма микрофонен ADC owner.

Цикличният тест: 16 цикъла за 204653 ms (204.653 s), AM/USB/LSB/FM по
четири пъти, GEN 50%. Всеки включва TX BACKEND/GEN SET, TX клавиатура,
RX възстановяване, RX клавиатура и нов TX. Няма MemoryError или блокирал
GUI callback; TX CLIP/deadline/FILE UND са 0. След четвъртия цикъл RX free
е 66848..66976 B (разлика 128 B, без монотонен спад); това включва RAM
тестовия сценарий. Проверени са CLK1 TX x1 и RX LO x4 регистрите, не
физическата честота. MIC/FILE, физически touch и многочасов soak остават.

Първият level сценарий мина цифровите assertions, но host завърши с
`Settings changed`: сравняваше всички 512 B вместо валидния 368-B SDR1
запис. Първоначалните два snapshots не са запазени и точната разлика
не може да се възстанови. Следващият прочит потвърди валидния запис срещу
предходния deployment архив. При поправените два теста before/after са
запазени; всички различни адреси са след края на записа, първите 368 B
са идентични. Не е обявявано повреждане или поправка на flash настройките.

Финален старт: firmware readback 1594288 B е идентичен; source SHA256
`f6bee007fede2dcd3d7e5673005791a8f7103eb6a228fc19645fac5343142467` и MPY
`d6cc332a76cf4377f38e123e590563a2efa5ea56c87a115aac67bacd25e45b98`
са потвърдени от файловете на платката. Оставено TX AM 579900 Hz,
GEN SINE 1000 Hz / 50%, TX LEVEL 40, depth 50. AF кадрите напредват;
последен status: AF51, DSP samples50101, CLIP/deadline/FILE UND/error 0,
двата DAC изхода enabled. Save callback е възстановен; след този старт
няма reset. Аналогово I/Q и RF потвърждение остава отделно.
...
- HOME използва същия десен бутон за MIC/R:xx, с контекстен надпис SOURCE
  вместо AGC при TX. Заглавието е SDR TRANSCEIVER при старт, RX и TX.
- TX BACKEND има SOURCE и LOOP/ONCE. Източникът на аудио и TX модулацията
  остават независими: R:USB не превключва автоматично предаването на USB.
- Общият picker се създава върху активния екран; при затваряне на BACKEND
  неговият picker се освобождава преди родителя. Pending операция блокира
  следващ избор, без втори native TX owner.
- LOOP променя поведението при следващ край на файла, без seek или нов owner.
  RX запазва стария guard; вече изпратен EOF не се отменя, а твърде кратък
  файл не може да включи LOOP по този път.
- След сигнал от потребителя за червени OVR/UND/CLIP: read-only проверката
  без reset показа TX USB/R:USB, спрян RX и стари RX флагове True/True/True.
  За 6 s ADC rails=0, DSP missed=0, CLIP=0; FILE UND331→331, AF3202→3342.
  `_poll_tx_status` рисуваше предупреждения само при FM, а `paint_status(None)`
  излизаше при празен `_st`, въпреки че RX consumer не попълва този cache.
- Поправени са и двата пътя: RX цветовете се чистят независимо от `_st`;
  всички TX режими рисуват AF/ADC/UND/CLIP. Червено означава ново събитие
  от предишния poll; следващ идентичен snapshot го изчиства. UND обхваща
  FILE starvation или пропуснат DSP срок. Общите броячи не се нулират;
  BACKEND показва отделно FILE UND, ADC rail, CLIP и missed.

Тестове по файлове:

- `test_sdr_tx_source_ui.py` — реалните Python методи с LVGL doubles:
  picker parent, callbacks, независим mode, pending, LOOP/EOF/short-file,
  изтриване на родител и възстановяване на RX обозначението.
- `test_sdr_tx_warnings.py` — реалните poll/painter методи за AM/USB/LSB/FM/CW,
  останали RX цветове, четири отделни брояча, identical-snapshot clear,
  запазени натрупани стойности и нулиране при смяна на owner.
- `test_sdr_fm_controls.py` — актуализиран TX mock и набор от widgets;
  `test_sdr_audio_controls.py` — очакваните BACKEND widgets;
  `test_sdr_tx_home.py` — новото заглавие;
  `test_sdr_tx_switch.py` — общ TX status painter вместо FM-only mock.
- Всичките шест host набора са PASS след окончателната поправка.
  Това не е проверка на физически touch, DAC или RF.

Качване: само app/MPY; native firmware не е rebuild-ван или flash-ван.
Преди първото обновяване е архивиран целият 16-MiB външен QSPI. Проверени са
64 предишни файла, непроменени internal firmware, валиден dataflash запис
и последните 4 MiB QSPI code. Последващите title/warnings обновявания пипат
само app файловете: старите са преименувани, новите са SHA256 readback-нати.
Няма тестови файлове във flash и IQ записите не са презаписвани.

Финален source318396 B SHA256
`de1704209a02d2e044aae4358664bf795ccf286a76c6d74c317c51c666510a90`;
MPY89313 B SHA256
`09b8a37036035547ebddfd9bb33694c9509b2364ac5a9e28050160a60f527d9d`.
Архив: `backups/tx-source-ui-1120000058-20260908-185551`.

Повторният цифров HIL през реални GUI CLICKED callbacks е PASS за
USB/LSB/AM/FM: SOURCE, LOOP→ONCE→LOOP, BACK без прекъсване на owner,
AF поток, I/Q DADR промяна (AM Q=2048), новият status ред и неактивни
червени флагове в установения интервал. Si5351 CLK1 регистрите съответстват
на TX×1 при 579400 Hz; това не е физическо измерване на CLK.
FILE UND общо USB833/LSB1241/AM679/FM2402; новите са 0 в кратките
установени интервали, DSP clips=0 и няма нови deadline misses. Преходните
пропуски не са отстранени. RAM тестът приключва с J-Link reset.
Физическо докосване, аналогова форма на DAC и RF качество не са измерени.
Протоколи: `tests/tx/results/tx-source-controls-20260908/`.
След reset на RAM теста нормалното приложение е оставено TX USB579400 Hz,
SOURCE R:USB, LOOP=True, LEVEL50; AF36→58, error/clip/deadline/FILE UND=0
в крайния кратък startup snapshot. Червените флагове са False/False/False
по runtime assertion. След този производствен старт не е изпълняван reset.

### CMS33: проверка на документация, не измерване

Прочетени са стр. 2.46–2.54 от R&S CMS33 Operator Manual, 840.0009.33 E-1
(локален `C:/Users/teodor/Documents/ChatGPT/various/tmp/pdfs/CMS33_Operator.pdf`).
RX-SSB е генератор за изпитване на приемник; TX-SSB измерва предавател.
Има USB/LSB/CW, еднотонен и двутонен тест, AF1/AF2 и RF нива LEV1/LEV2.
При един тон ръководството описва потисната носеща и нежелана лента.
При два тона нежеланите съставки НЕ са потиснати: отместване с 10 kHz
ги отдалечава от желаната лента. В този режим MODGEN е с 10 kHz над
показаната аудиочестота (стр. 2.52). AF2 OFF, не LEV2 OFF, изключва втория тон.
Не е потвърдена генерация на произволен SSB глас/файл. За нашите DAC I/Q
е нужен външен RF модулатор преди RF входа на CMS; той не заменя осцилоскопа.

## 2026-09-08: достъп до R:USB през TX HOME — преди поправката

Прочетено текущото приложение без reset: TX LSB579400 Hz, MIC, LOOP=True,
няма pending операция или грешка. R:USB присъства в наличните профили.
HOME бутонът agc-pill показва MIC: clickable, видим, не е DISABLED.
Над него обаче остава постоянният надпис AGC; TX BACKEND показва входа
само като текст, без контрол за избор. Това е проблем с обозначаването
и откриваемостта; отказ на физическия touch не е доказан с тази проверка.

През еднократен GUI timer е изпратен CLICKED към съществуващия MIC бутон.
Регистрираният callback отвори TX AUDIO SOURCE: MIC, R:AM, R:FM, R:USB,
R:LSB; всички са enabled. R:USB е на y130, височина30 в панела. Менюто
е оставено отворено за потребителя. Източникът НЕ е сменен и LSB579400 Hz
е запазен; не са качвани файлове, firmware или тестове и няма reset.

Инструкция: HOME → MIC под десния плъзгач → R:USB. Това избира аудиото
от USB записа, но не сменя TX модулацията: тя се избира отделно от долния
HOME бутон. За предаване на USB трябва отделно да се избере USB.
Протокол: tests/tx/results/tx-source-ui-20260908.log.

## 2026-09-08: „I/Q изходите са тихи“ — прочит без reset

Диагностика върху текущото приложение, без промяна на вход, режим,
параметри или DAC регистри; без reset/flash. TX AM579900 Hz, MIC/P001,
file_source=False, _inj_source=0, _inj_on=False; няма активен GEN/FILE.

24 редки status снимки през 50 ms: DADR I2366..2404 (размах38 кода),
Q2048..2048, last_adc1140..1517. За целия диагностичен интервал
dsp_samples +63509 и AF +34; error/deadline/clips=0. Полетата i_code/q_code
се четат от DADR0/DADR1 в ra_tx_hw_get_status(); не са аналогово измерване
и не представляват атомарна I/Q двойка. Размахът е от редките снимки,
не доказан пълен пиков обхват или честота на сигнала.

ra_tx_hw.c:tx_cpu_sample() задава Q=q_zero при AM; това не е спрял Q DAC.
ra_tx_core_am_sample(): при gain100%, depth50%, amplitude409, входен
размах377 кода дава приблизително 377×0.5×409/2048=37.65 изходни кода,
съгласувано с наблюдаваните38. Няма включен известен тестов тон.
Предишният финал остави MIC, а не R:AM/R:USB — броячите AF сами по себе си
не доказват подходящ сигнал за проверка на двата физически изхода.

Следва избор на известен източник: R:AM проверява AM обвивката на I при
постоянен Q; R:USB/SSB позволява наблюдение на променливи I и Q. Не е
сменен автоматично режимът/източникът. Ако на пиновете липсва и измеримото
постоянно ниво, регистровите данни не изключват физически проблем.
Протокол: tests/tx/results/tx-quiet-readonly-20260908.log.
