...
## 2026-09-01 22:11 +03:00 - Build 33 flash, Flex timing HIL and complete demo catalog

- Git milestone before flashing: parent port `f9da261ce48c6e0f301263fe4322f3e8adb80dd9`; binding remained `607c39780d489989c300dc6e3f36d30ca884de91`; LVGL remained `dccff2ee12ea1431ed36d6fbafb4f4d7bddcc5d5`.
- The required separate visible J-Link reset completed with exit code 0. Build 33 was then programmed in a visible J-Link session, followed by a separate visible `verifybin` session; both exited with code 0. The checked image is exactly `1,572,568` bytes with SHA-256 `34EA142F2E9C6F18B4CAB56824EC14BA0176650A8E9F4BB69878719DA3342918`. The temporary verify command file was removed.
- One initial `mpremote` attempt did not execute board code because Thonny's `bare_metal_backend.py` held COM18. Only that dedicated backend process was stopped; the Thonny GUI and files were left open. A new visible J-Link reset preceded the real test.
- The Flex diagnostic no longer contains the experimental Python `RENDER_START` callback or fixed `6,500 us` delay. It relies only on the Build 33 C scheduler and runs two top horizontal, one bottom horizontal and two vertical cases.
- Top horizontal result: both independent cases applied a calculated `6,872 us` delay in all `60/60` renders. Actual render duration was `7,669..8,466 us`, dirty bounds were `y=0..84`, and frame crossings, VSYNC timeouts and no-window counts were all zero.
- Other-direction result: the bottom horizontal and both vertical cases applied no beam delay. Their maximum render durations were `12,278 us`, `11,947 us` and `13,079 us`; frame crossings and VSYNC timeouts remained zero. The vertical cases correctly reported no safe delayed window rather than applying an unsafe wait.
- Complete official catalog: `run_official_hil.py` executed all `162/162` standalone LVGL 9.4 MicroPython files from RAM between `22:03:37+03:00` and `22:09:18+03:00`. Every file received a separate visible J-Link reset. Duration was `341.344 s`; result was `159 PASS`, `3 SKIP_UNSUPPORTED`, `0 FAIL`, zero nonzero return codes and zero recorded errors.
- The three expected SKIP records are exactly `lv_example_scroll_5`, `lv_example_label_3` and `lv_example_label_7`. The two changed markers are the dynamic nonzero-pixel counts of `lv_example_lottie_1` and `lv_example_lottie_2`; both remained valid PASS results with the same `5,139`-byte JSON source and nonempty output.
- Regression-sensitive examples passed: all Flex, Grid and supported Scroll examples; both Lottie examples; and the previously sensitive `lv_example_msgbox_2`, which completed in `1.69 s` without timeout.
- Host and site follow-up: all `162` official files passed syntax and full host-smoke execution; the `11` project lessons passed syntax with seven host-smoke and four explicitly hardware-only cases, with zero failures. The regenerated site contains `173` entries. Playwright desktop/mobile QA reported `0` console/page errors and no horizontal overflow.
- Result: **PASS** for Build 33 firmware transfer/verify, automatic beam scheduling, complete official demo HIL and host/site regression checks. A fresh user visual confirmation of the automatic no-flicker result remains separate from these timing counters; the earlier controlled manual-delay comparison was visually confirmed.

## 2026-09-01 23:42 +03:00 - Partial staging buffer, build 34 attempt 1

- Visual trigger: subsequent user observation showed that left/right movement still flickers on Build 33, although the main demo was reported as working correctly. The Build 33 timing counters therefore do not constitute visual proof that horizontal tearing is removed.
- Selected architecture: keep Dave2D enabled, replace direct rendering into the visible single framebuffer with one static 60-line RGB565 partial draw buffer, and copy each completed area to the physical framebuffer only after GLCDC line detect. No draw-buffer allocation is performed during frame processing.
- Command: `make BOARD=VK_RA6M3 clean`, then `make BOARD=VK_RA6M3 -j16` in the MinGW64 environment with `-j16` exactly.
- Compile result: all generators completed; the LVGL core, Dave2D sources, ThorVG sources, board `dave2d_port.c` and modified `machine_lcd.c` compiled. The final link failed before producing a flashable image.
- Exact linker failure: the existing FSP/MicroPython heap remained fixed at `0x47000` (`290,816`) bytes while the new `.noinit` draw buffer added `0xE100` (`57,600`) bytes. `.heap` and `.stack_dummy` crossed the `FB0` origin, producing `RAM sections overlap framebuffer` and `region RAM overflowed by 54456 bytes`.
- Consequence: this image was not committed, flashed or run. The attached board remains on the previously verified Build 33 image.
- Correction selected for attempt 2: retain the existing VK_RA6M3 fixed-heap/linker model, define the partial-buffer line count and byte reservation once in the board configuration, derive `BSP_CFG_HEAP_BYTES` from that reservation, and assert in `machine_lcd.c` that the generated GLCDC stride produces the identical byte count. This avoids an unrelated linker-model change and turns any future stride/configuration mismatch into a compile-time failure.
- Runtime acceptance remains separate: the partial path must prove its flush-copy duration and multi-area refresh rate on hardware, and the currently working dual-thermal demo must retain the required 10 FPS before this can replace Build 33.

## 2026-09-01 23:59 +03:00 - Partial staging buffer, build 34 attempt 2

- Command: a successful `make BOARD=VK_RA6M3 clean` was followed by `make BOARD=VK_RA6M3 -j16` in the MinGW64 environment. One earlier direct-Bash `clean` invocation lacked MinGW64 on `PATH`, returned `make: command not found`, and changed nothing; it was not a compilation attempt.
- Result: **SUCCESS** (`make` exit code 0). Linking completed and generated `firmware.elf`, `firmware.hex` and `firmware.bin`.
- Size summary: `text=1572325`, `data=0`, `bss=647732`, total `2220057` bytes (`0x21e019`). `firmware.bin` is `1,572,312` bytes with SHA-256 `C59271B402BC2EB616867F97E681335BAE0CB96FD573D77FE07EE09B23B00354`.
- Delta from Build 33: executable text and binary length decreased by `256` bytes; the aggregate BSS-class total increased by only `28` bytes because the new `0xE100` staging reservation is balanced by an equal reduction of the fixed MicroPython heap.
- RAM layout: `s_lcd_lvgl_draw_buffer` is `0xE100` (`57,600`) bytes at `0x1FFE0F40`, aligned to 64 bytes. The MicroPython heap is `0x38F00` (`233,216`) bytes at `0x200024B8..0x2003B3B8`; the stack is `0x4000` (`16,384`) bytes at `0x2003B3B8..0x2003F3B8`; `0xC48` (`3,144`) bytes remain before framebuffer origin `0x20040000`. `FB0` remains exactly `0x3FC00` (`261,120`) bytes.
- Configuration consistency: the board configuration defines `60` staging lines and `0xE100` reserved bytes, derives the heap size from that reservation, and `machine_lcd.c` statically asserts that the generated GLCDC stride produces the same byte count. The assertion compiled successfully.
- Rendering path: LVGL is registered in `LV_DISPLAY_RENDER_MODE_PARTIAL`. Dave2D renders each partial area into the static buffer; the C flush callback waits for the next GLCDC line-detect event, copies RGB565 rows into the physical framebuffer, executes a memory barrier and calls `lv_display_flush_ready()`. Disassembly retains the bounded VSYNC wait, row-wise `memcpy` loop and flush-ready call.
- Dave2D proof: `lv_draw_dave2d_init`, `d2_opendevice`, `d2_executerenderbuffer`, `d2_flushframe`, `drw_int_isr` and the VK_RA6M3 DRW ISR remain linked. The known float-coordinate clip warnings remain visible but non-fatal.
- ABI and segments: the ELF remains ARM v7E-M, VFPv4-D16 hard-float with VFP argument registers. Executable LOAD segments are `R E`, writable segments are `RW`, and no LOAD segment is RWX.
...
## 2026-09-02 02:58 +03:00 - Build 36 visual touch acceptance

- A subsequent user-operated check of the Build 36 image was reported as `сега е ок`, accepting the current touch behavior after the rejected Build 35 flicker-and-lag result.
- The supporting automatic horizontal Flex comparison completed `48` renders in each of the NORMAL and PRESSED phases. Maximum measured render duration was `10,337 us` and `10,657 us` respectively; both phases used the calculated `6,872 us` beam delay and reported zero no-window cases.
- No source, build artifact or board image changed after the independently verified Build 36 transfer. The accepted board image remains the `1,572,744`-byte binary with SHA-256 `06997FF22AB4594430A3EC04A3C0D3F9121B55ED4D6C1F0C30D6893658ABF61E` from commit `e0b88bcef`.
- Result: **PASS** for the Build 36 horizontal touch visual milestone. Thermal-camera image quality and dual-sensor FPS remain separate pending regressions.

## 2026-09-02 10:54 +03:00 - Build 36 thermal, Flex and complete-catalog regression

- Scope: no firmware source, binary or final thermal application was changed. All diagnostics were separate scripts executed from RAM. The board remains on commit `e0b88bcef` and the independently verified `1,572,744`-byte Build 36 image with SHA-256 `06997FF22AB4594430A3EC04A3C0D3F9121B55ED4D6C1F0C30D6893658ABF61E`.
- Ten-second thermal production check: `PRODUCTION_COUNTS 100 100 121 54 30 35 0` and `PRODUCTION_STATE False 0 0 True`; the reported temperature ranges remained valid.
- Thermal framebuffer check: MLX RAW contained `6,900/6,912` nonzero RGB565 pixels and MLX bilinear contained `6,912/6,912`. All four rendered-buffer/framebuffer mismatch counters were zero. This directly rejects the previously observed all-black MLX interpolation state for the tested frames.
- Sixty-second thermal soak: `SOAK_COUNTS 600 600 736 322 180 0 202`, `SOAK_FPS 9.999166 9.999166 12.265644`, update failure `False`, I2C errors `0/0`, invalid-frame counters `0/0/0`, heap-lock depth `0`, and DTC statistics `(3, 0, 0, 0, 2, 0, 0, 0)`.
- FPS interpretation: the display update and AMG8833 read paths sustained approximately `10.0 FPS`. The MLX90640 counter sustained `12.266` subpages/s, while `322` complementary subpage pairs in approximately `60.005 s` equal about `5.37` complete MLX fields/s. The display therefore refreshes at 10 FPS using the latest valid merged MLX field; this result must not be described as 10 newly completed MLX fields/s.
- Timer-driven top-horizontal Flex diagnostic: `160` steps produced `95` renders. Every render used the calculated `6,872 us` beam delay, with measured render time up to `10,343 us`, zero no-window cases and zero VSYNC timeouts. The script ended with `FLEX_TIMER_SCROLL_DIAGNOSTIC_READY`.
- Five-case Flex diagnostic: both top-horizontal runs completed `60` renders with the `6,872 us` delay and zero VSYNC timeouts. Bottom-horizontal and both vertical cases also completed `60` renders each with zero VSYNC timeouts; they correctly reported no guaranteed delayed window where applicable. The script ended with `FLEX_SCROLL_DIAGNOSTIC_READY`.
- Complete official LVGL catalog: `run_official_hil.py` executed all `162/162` standalone MicroPython examples from RAM between `10:47:14+03:00` and `10:52:45+03:00`. The harness performed a separate visible J-Link reset before every example. Duration was `330.844 s`; result was `159 PASS`, `3 SKIP_UNSUPPORTED`, `0 FAIL`.
- Expected unsupported records remained exactly `lv_example_scroll_5`, `lv_example_label_3` and `lv_example_label_7`. The only dynamic marker change was `lv_example_lottie_1`, whose nonzero intermediate-buffer byte count changed from `780` to `772`; the complete `5,139`-byte resource loaded and the example remained PASS. That count is diagnostic, not a golden rendering value.
- User-visible boundary: the user separately reported the horizontal/vertical Flex demo as `много е добре`. The automatic catalog verifies startup and declared markers for every example, but does not synthesize every possible touch gesture inside all 162 examples.
- Result: **PASS** for the Build 36 thermal regression, targeted Flex timing diagnostics and complete official LVGL example catalog. The accepted final firmware/application remains unchanged; only standalone diagnostics and result records were updated.

## 2026-09-05 02:31 +03:00 - Build 37 Dave2D rounded-border geometry guard

- Trigger: the exact `lv_example_event_draw.py` produced a persistent `0xFF5555` horizontal band outside the animated circular object. Pausing the timer did not remove it, while a full-screen redraw after disabling the border did, proving framebuffer contamination rather than capture timing.
- Root cause: with a `2 px` circular border, object sizes `2x2` and `3x3` make the straight rounded-border core width `-2` and `-1` pixels. The bundled D/AVE `d2_renderbox_inline()` rejection casts width to `d2_u16`, so a negative width is not treated as an empty box and can reach the hardware command list.
- Correction: `lv_draw_dave2d_border.c` now submits each straight border segment only when both area axes are ordered. Valid boxes, all rounded corner wedges and the complete Dave2D draw unit remain active; there is no software-rendering fallback and the Python example is unchanged.
- Command: a successful `make -C /home/teodor/renesas_micropython/ports/renesas-ra BOARD=VK_RA6M3 clean` was followed by `make -C /home/teodor/renesas_micropython/ports/renesas-ra BOARD=VK_RA6M3 -j16` in the MinGW64 environment. A preceding clean attempt without `-C` ran in the MSYS home directory, returned `No rule to make target 'clean'`, and performed no clean or compilation.
