 ...
 * Copyright (c) 2026
 *
 * Phase 1 Observation — counter storage, atomic primitives, 64-bit
 * seqlock helpers, and reset prototype for the Renesas LoRaWAN C-stack
 * port.
 *
 ...
 * call site to ((void)0). The g_lorawan_stats storage and the Python
 * mac.stats() / mac.stats_reset() methods still compile and link — Step
 * 3 §7 contract: storage stays, methods compile, dict reads as zeros and
 * reset is a no-op. This is the reference build for Step 14 build #4
 * size comparison, and the public API surface is preserved so application
 * code does not need to be conditional.
 *
 ...
 * afterwards — no atomics required. Consumers (mac.stats() under Step 12)
 * may skip cycle-max reporting / re-enable attempts when false. */
 ...
 * nvm_save_in_rx_window_count over-counts. Phase 1 accepts this; Phase 3 will
 * tighten with dedicated RX-arm/disarm hooks.
 *
...
typedef struct lorawan_stats {
    ...
    /* AD5.7 — per-stage micro-timing. spi_stage_pre_busy_max_us and
       ...
       analysis can attribute wall-time. spi_stage_dtc_max_us covers the
       yield loop between ra_sci_spi_submit return and SPI_CMPL_DTC_DONE.
       Both busy_wait_*_us and the new stage counters coexist. */
    uint32_t spi_stage_pre_busy_max_us;
    uint32_t spi_stage_dtc_max_us;
    ...
    uint8_t  busy_timeout_opcode;       /* sentinel 0xFF = invalid */
    uint8_t  busy_last_opcode;          /* sentinel 0xFF = invalid */
    uint8_t  _pad_busy[2];              /* keep next u32 4-byte aligned */

    /* group: isr */
    uint32_t hard_isr_dio1_count;
    uint32_t hard_isr_agt4_count;
    uint32_t hard_isr_agt5_count;
    uint32_t hard_isr_queue_overflow_count;
    uint32_t hard_isr_queue_push_count;
    uint32_t hard_isr_dio1_cycles_max;
    uint32_t hard_isr_agt4_cycles_max;
    uint32_t hard_isr_agt5_cycles_max;
    uint32_t hard_isr_dio1_reentry_count;
    uint32_t hard_isr_agt4_reentry_count;
    uint32_t hard_isr_agt5_reentry_count;
    /* AD5.2 — DTC done-callback + BUSY-falling ICU augmentation.
       AD5.2 defines + exposes; counters stay 0 until AD5.3 (DTC) /
       AD5.4 (BUSY ICU) wire the callbacks into the LoRa SPI path. */
    uint32_t hard_isr_dtc_count;
    uint32_t hard_isr_dtc_cycles_max;
 ...
 * context). No atomic needed — the value is only consumed by mac.stats()
 * which never preempts the writer.
 *
...
/* Hard-ISR cycle-delta wrapper. Used by DIO1 / AGT4 / AGT5 hard IRQs to
 ...
 * itself. The reader (mac.stats() in Python/scheduler context) twin-reads
 * the high half and retries until it observes a stable epoch. See Step 3
 * §3 for the full justification.
 */
...
static inline uint64_t stats_spi_bytes_read(void) {
    const uint32_t *p_lo = (const uint32_t *)&g_lorawan_stats.spi_bytes_total;
    const uint32_t *p_hi = p_lo + 1;
    uint32_t hi1, hi2, lo;
    do {
        hi1 = __atomic_load_n(p_hi, __ATOMIC_RELAXED);
        lo  = __atomic_load_n(p_lo, __ATOMIC_RELAXED);
        hi2 = __atomic_load_n(p_hi, __ATOMIC_RELAXED);
    } while (hi1 != hi2);
    return ((uint64_t)hi1 << 32) | lo;
}

/* Phase 1 lifecycle prototypes. Reset is implemented in mod_lorawan.c
 * alongside the storage definition; the DWT enable helper lands with
 * Step 9 ISR cycle counters. */
