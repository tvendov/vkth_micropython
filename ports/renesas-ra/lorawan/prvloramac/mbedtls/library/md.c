 ...
 */
  ...
  *
  * Changes:
  *   - Added hmac_sha1_vector and md_hmac_vector_sha1 based on mbedtls_md_hmac()
  *   - Disabled mbedtls_md_hmac, mbedtls_md_free, mbedtls_md_setup to reduce rom/ram size
 */

#include "common.h"

#if defined(MBEDTLS_SHA1_C) && defined(MD_HMAC_SHA1_ONLY)
extern void mbedtls_xor(unsigned char *r, const unsigned char *a, const unsigned char *b, size_t n);
#endif
...
#if !defined(MD_HMAC_SHA1_ONLY) && !defined(MD_HMAC_SHA1_ALLOC_CTX_IN_STACK)
...
#if defined(MD_HMAC_SHA1_ONLY) && defined(MD_HMAC_SHA1_ALLOC_CTX_IN_STACK)
/*
 *  Note: This funcion refers to the function mbedtls_md_hmac()
*/
static int md_hmac_vector_sha1(const mbedtls_md_info_t *md_info,
                    const unsigned char *key, size_t keylen,
                    size_t                   num_elem,
                    const unsigned char *input[], const size_t *ilen,
                    unsigned char *output)
{
    mbedtls_md_context_t ctx;
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t      i;
#if defined(MD_HMAC_SHA1_ALLOC_CTX_IN_STACK)    // Added to allocate context area from the stack for MD_SHA1
    unsigned long sha1_ctx[(2 + 5 + (64 / 4))];   // struct mbedtls_sha1_context
    unsigned char hmac_sha1_ctx[64 * 2];        // block size(64) * 2
#endif
    ...
    if (md_info->type == MBEDTLS_MD_SHA1) {
        ctx.md_ctx = sha1_ctx;
        ctx.hmac_ctx = hmac_sha1_ctx;    // mbedtls_calloc( 2, md_info->block_size );
        ctx.md_info = md_info;
    }
#else
    if ((ret = mbedtls_md_setup(&ctx, md_info, 1)) != 0) {
    ...
    {
        if ((ret = mbedtls_md_hmac_update(&ctx, input[i], ilen[i])) != 0) {
            goto cleanup;
        }
    }
    if ((ret = mbedtls_md_hmac_finish(&ctx, output)) != 0) {
...
cleanup:
    ...
    if (md_info->type == MBEDTLS_MD_SHA1) {
        ...
        mbedtls_platform_zeroize(hmac_sha1_ctx, sizeof(hmac_sha1_ctx));
...
int hmac_sha1_vector(const uint8_t* key, size_t key_len, size_t num_elem, const uint8_t* data[], const size_t* len, uint8_t* mac)
{
    return md_hmac_vector_sha1(mbedtls_md_info_from_type(MBEDTLS_MD_SHA1), key, key_len, num_elem, data, len, mac);
}
