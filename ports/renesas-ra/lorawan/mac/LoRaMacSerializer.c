...
LoRaMacSerializerStatus_t LoRaMacSerializerJoinRequest( LoRaMacMessageJoinRequest_t* macMsg )
{
    ...
    LoRaMacCommonCopyUint32ToArray( &(macMsg->Buffer[bufItr]), &(macMsg->MIC), 4 );
    bufItr += 4;
...
LoRaMacSerializerStatus_t LoRaMacSerializerData( LoRaMacMessageData_t* macMsg )
{
    ...
    LoRaMacCommonCopyUint32ToArray( &(macMsg->Buffer[bufItr]), &(macMsg->FHDR.DevAddr), 4 );
    bufItr += 4;
    ...
    LoRaMacSerializerDataSetMic( &( macMsg->Buffer[bufItr] ), macMsg->MIC );
    bufItr += LORAMAC_MIC_FIELD_SIZE;

    macMsg->BufSize = bufItr;
    return LORAMAC_SERIALIZER_SUCCESS;
}

void LoRaMacSerializerDataSetMic( uint8_t *p_buffer, uint32_t mic )
{
    LoRaMacCommonCopyUint32ToArray( p_buffer, &mic, 4 );
}
