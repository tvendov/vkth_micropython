...
typedef struct sLoRaMacParams
{
    ...
    float AntennaGain;
    /*!
     * CCA configuration for AS923.
     * CCA is enabled for AS923 when true, and disabled otherwise.
     */
    bool EnableCca;
}LoRaMacParams_t;
...
typedef enum eLoRaMacEventInfoStatus
{
    ...
    /*!
     * An error occurred in the join procedure (same AppNonce(LW103)/JoinNonce(LW104))
     */
    LORAMAC_EVENT_INFO_STATUS_JOIN_NONCE_FAIL,
    /*!
    ...
    /*!
     * Successfully received a beacon frame
     */
    LORAMAC_EVENT_INFO_STATUS_BEACON_LOCKED,
    /*!
     * Could not receive a beacon frame
     */
    LORAMAC_EVENT_INFO_STATUS_BEACON_LOST,
    /*!
     * Beacon acquisition failure
     */
    LORAMAC_EVENT_INFO_STATUS_BEACON_NOT_FOUND,
    /*---*/
    MAXNUM_LORAMAC_EVENT_INFO_STATUS,
...
typedef enum eMib
{
     ...
     * CCA is enabled for AS923 when true, and disabled otherwise.
     */
    MIB_AS923_ENABLE_CCA,
    /*!
     * Ping slot Periodicity
     */
    MIB_PING_SLOT_PERIODICITY,
    /*!
    ...
    MIB_RX1_DROFFSET,
    MIB_MAX_EIRP,
    MIB_DOWNLINK_DWELLTIME,
    MIB_UPLINK_DWELLTIME,
    MIB_DOWNLINK_FCNT,
    MIB_UPLINK_FCNT,
}Mib_t;

// convert MIB
#define MIB_NWK_SKEY        MIB_NWK_S_ENC_KEY
#define MIB_APP_SKEY        MIB_APP_S_KEY
#define MIB_APP_EUI         MIB_JOIN_EUI
#define MIB_APP_ROOT_KEY    MIB_NWK_KEY
...
typedef union uMibParam
{
     ...
     * CCA is enabled for AS923 when true, and disabled otherwise.
     *
     * Related MIB type : \ref MIB_AS923_ENABLE_CCA
     */
    bool EnableCca;
    /*!
     * duty cycle enabled
     ...
     *
     * Related MIB type: \ref MIB_APP_EUI
     */
    uint8_t* AppEui;
    /*!
     * Applicaiton root key
     *
     * Related MIB type: \ref MIB_APP_ROOT_KEY
     */
    uint8_t* AppRootKey;
    /*!
     * Network session key
     ...
     * Ping slot Periodicity
     *
     * Related MIB type: \ref MIB_PING_SLOT_PERIODICITY
     */
    ...
    uint16_t devNonce;          // MIB_DEV_NONCE
    uint32_t appNonce;          // MIB_APP_NONCE (JoinNonce)
    uint8_t maxDcycle;          // MIB_MAX_DCYCLE
    uint8_t rx1DrOffset;        // MIB_RX1_DROFFSET
    uint8_t maxEirp;            // MIB_MAX_EIRP
    uint8_t downlinkDwellTime;  // MIB_DOWNLINK_DWELLTIME
    uint8_t uplinkDwellTime;    // MIB_UPLINK_DWELLTIME
    uint32_t downlinkFCnt;      // MIB_DOWNLINK_FCNT
    uint32_t uplinkFCnt;        // MIB_UPLINK_FCNT
}MibParam_t;
...
typedef enum eLoRaMacStatus
{
    ...
    /*!
     * Transmission was aborted due to duty cycle restriction
     */
    LORAMAC_STATUS_DUTYCYCLE_RESTRICTED,
    /*!
     * Data rate not supported by any channel
     */
    LORAMAC_STATUS_NO_CHANNEL_FOUND,
    /*!
     * No free channel found by carrier sense.
     */
    LORAMAC_STATUS_NO_FREE_CHANNEL_FOUND,
#ifdef LORAMAC_CLASSB_ENABLED
     /*!
      * Transmission was aborted due to overlap with beacon reserved time
      */
    LORAMAC_STATUS_BUSY_BEACON_RESERVED_TIME,
     /*!
      * Transmission was aborted due to an overlapping ping slot
      */
    LORAMAC_STATUS_BUSY_PING_SLOT_WINDOW_TIME,
     /*!
      * Transmission was aborted due to overlap with beacon operation timing.
      */
     ...
     */
    LORAMAC_STATUS_RADIO_FAIL,
    /*!
     * Radio parameter setting invalid
     */
    LORAMAC_STATUS_RADIO_PARAMETER_INVALID,
    /*-----*/
    MAXNUM_LORAMAC_STATUS,
}LoRaMacStatus_t;
...
typedef enum eLoRaMacRegion_t
{
    ...
    /*!
     * AS band on 923MHz (Group2,3(LW104) and Japan)
     */
    LORAMAC_REGION_AS923_JPN,
#if (LORAMAC_VERSION >= LORAWAN_VERSION_1_0_4)
    LORAMAC_REGION_AS923_2,
    LORAMAC_REGION_AS923_3,
    LORAMAC_REGION_AS923_4,
    LORAMAC_REGION_AS923_MAXGROUP,  // indicate the max group only
#endif
}LoRaMacRegion_t;
...
/*!
 * Enumeration of LoRaMac error notification status
 */
typedef enum LoRaMacErrorNotificationStatus_e
{
    /*!
     * Radio RX configuration failure
     */
    LORAMAC_ERROR_NOTIFICATION_STATUS_RADIO_CHECK_FAIL_RX_CFG = 100

}LoRaMacErrorNotificationStatus_t;
...
typedef struct sLoRaMacCallback
{
    ...
    /*!
     *\brief    Will be called to notify error
     *
     */
...
/*!
 * \brief   LoRaMAC Low Power
 */
LoRaMacStatus_t LoRaMacSetLowPower( void );

/*!
 * \brief   This is an internal function to fetch the value of one MIB variable.
 */
void LoRaMacGetMibVal( Mib_t varType, void * pDest );

/*!
 * \brief   This is an internal function to notify LoRaMac error
 */
void LoRaMacErrorNotify( LoRaMacErrorNotificationStatus_t status );

#if defined( LORACOMBO_ENABLED )
/*!
 * \brief   This is an internal function to query LoRaMac is started.
 */
...
/*-----------------------------------------------------------*/
/*! 
 * Macros for RFIC low power
 */
#define LORAMAC_RADIOSLEEP_TXDONE( x )  \
{\
    Radio.SleepWarm();\
}

#ifdef LORAMAC_CLASSB_ENABLED
    #define LORAMAC_RADIOSLEEP_RXDONE( x )  \
    {\
        ...
        {\
            if( ( LoRaMacClassBIsAcquisitionPending() == true ) &&\
                ( Radio.GetStatus() == RF_RX_RUNNING ) )\
            {\
                /* BeaconAcquisition (continuousRx): no-op */\
            }\
            else\
            {\
                if( LoRaMacClassBIsReadyRxSlots() == true )\
                {\
                    /* PingSlot periodicity is short. */\
                    Radio.SleepWarm();\
                }\
                else\
                {\
...
#define LORAMAC_RADIOSLEEP_RXDONE_BEACON( x )  \
{\
    if( ( LoRaMacClassBIsAcquisitionPending() == true ) &&\
        ( Radio.GetStatus() == RF_RX_RUNNING ) )\
    {\
...
#ifdef LORAMAC_CLASSB_ENABLED
    #define LORAMAC_RADIOSLEEP_TXTIMEOUT( x )   \
    {\
        if( MacCtx.NvmCtx->DeviceClass == CLASS_C )\
        {\
            /* ClassC: no-op */\
...
#ifdef LORAMAC_CLASSB_ENABLED
    #define LORAMAC_RADIOSLEEP_RXERRTO( x ) \
    {\
        if( MacCtx.NvmCtx->DeviceClass == CLASS_C )\
        {\
            /* ClassC: no-op */\
        ...
        {\
            ...
            {\
                ...
                {\
                    Radio.Standby(); /* safety for continuous rx */ \
                    Radio.SleepCold();\
                }\
            }\
        }\
...
#define LORAMAC_RADIOSLEEP_RXERRTO_RX1ERR_ABORT( x ) \
{\
    Radio.WakeUp();\
    Radio.Standby();\
    Radio.SleepCold();\
...
#define LORAMAC_RADIOSLEEP_SWITCHCLASS_C2A( x ) \
{\
    Radio.Standby();\
    Radio.SleepCold();\
}
...
#define LORAMAC_RADIOSLEEP_MACINIT( x ) \
{\
    Radio.SleepCold();\
}

#define LORAMAC_RADIOSLEEP_BEACON_NOTFOUND( x ) \
{\
    Radio.Standby(); /* safety for continuous rx */ \
    Radio.SleepCold();\
}

#define LORAMAC_RADIOSLEEP_TXFAILED( x )    \
{\
...
#define LORAMAC_RADIOSLEEP_BEACON_LOST( x )    \
{\
    if( Radio.GetStatus() != RF_COLD_SLEEP )\
    {\
        Radio.WakeUp();\
        Radio.Standby();\
        Radio.SleepCold();\
...
#define LORAMAC_RADIOSLEEP_BEACON_STOP( x )    \
{\
    if( Radio.GetStatus() != RF_COLD_SLEEP )\
    {\
        Radio.WakeUp();\
...
/*! 
 * Macros for RFIC wake up / standby
 */
#define LORAMAC_RADIOWAKEUP_TX( x )    \
{\
    if( MacCtx.NvmCtx->DeviceClass != CLASS_C )\
    {\
        Radio.WakeUp();\
...
#define LORAMAC_RADIOWAKEUP_RXWIN( x )    \
{\
    Radio.WakeUp();\
    Radio.Standby();\
}

#define LORAMAC_RADIOWAKEUP_RXC( x )    \
{\
    Radio.WakeUp();\
    Radio.Standby();\
    Radio.SetPublicNetwork( MacCtx.NvmCtx->PublicNetwork );\
...
#define LORAMAC_RADIOWAKEUP_BEACON( x )    \
{\
    bool publicNwk;\
    LoRaMacGetMibVal( MIB_PUBLIC_NETWORK, &publicNwk );\
    \
    Radio.WakeUp();\
    Radio.Standby();\
    Radio.SetPublicNetwork( publicNwk );\
}

#define LORAMAC_RADIOWAKEUP_PINGSLOT( x )    \
{\
    bool    publicNwk;\
    LoRaMacGetMibVal( MIB_PUBLIC_NETWORK, &publicNwk );\
    \
...
#define LORAMAC_RADIOWAKEUP_MULTICASTSLOT( x )    \
{\
    bool    publicNwk;\
    LoRaMacGetMibVal( MIB_PUBLIC_NETWORK, &publicNwk );\
    \
...
/*-----------------------------------------------------------*/
#define LORAMAC_NVM_MIBFLG_DEV_NONCE            0x00000001
#define LORAMAC_NVM_MIBFLG_APP_NONCE            0x00000002
#ifdef LORAMAC_ACTMODE_ABP_ENABLED
#define LORAMAC_NVM_MIBFLG_DOWNLINK_FCNT        0x00000004
#define LORAMAC_NVM_MIBFLG_UPLINK_FCNT          0x00000008
#define LORAMAC_NVM_MIBFLG_CHANNELS             0x00000010
#define LORAMAC_NVM_MIBFLG_CHANNELS_MASK        0x00000020
#define LORAMAC_NVM_MIBFLG_CHANNELS_DATARATE    0x00000040
#define LORAMAC_NVM_MIBFLG_CHANNELS_NB_TRANS    0x00000080
#define LORAMAC_NVM_MIBFLG_CHANNELS_TXPOWER     0x00000100
#define LORAMAC_NVM_MIBFLG_MAX_DCYCLE           0x00000200
#define LORAMAC_NVM_MIBFLG_RX1_DROFFSET         0x00000400
#define LORAMAC_NVM_MIBFLG_RX2_FREQUENCY        0x00000800
#define LORAMAC_NVM_MIBFLG_RX2_DATARATE         0x00001000
#define LORAMAC_NVM_MIBFLG_RECEIVE_DELAY_1      0x00002000
#define LORAMAC_NVM_MIBFLG_MAX_EIRP             0x00004000
#define LORAMAC_NVM_MIBFLG_DOWNLINK_DWELLTIME   0x00008000
#define LORAMAC_NVM_MIBFLG_UPLINK_DWELLTIME     0x00010000
#define LORAMAC_NVM_MIBFLG_PING_SLOT_DATARATE   0x00020000
#endif
//----
#ifdef LORAMAC_ACTMODE_ABP_ENABLED
#define LORAMAC_NVM_MIBFLG_ALL                  0x00000003
#else
#define LORAMAC_NVM_MIBFLG_ALL                  0x0003FFFF
#endif
