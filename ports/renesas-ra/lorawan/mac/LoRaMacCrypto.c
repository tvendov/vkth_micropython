...
/*
 * Initial value of the DevNonce (if DevNonce is incremental)
 */
#if (LORAMAC_USE_RANDOM_DEV_NONCE == 0)
#define DEVNONCE_INITAL_VALUE           (uint16_t)0xFFFF
#endif
...
typedef struct sLoRaMacCryptoNvmCtx
{
     ...
     */
    FCntList_t FCntList;
#if ( USE_LRWAN_1_1_X_CRYPTO == 1 )  // LW11x
    /*
...
static LoRaMacCryptoStatus_t PayloadEncrypt( uint8_t* buffer, int16_t size, KeyIdentifier_t keyID, uint32_t address, uint8_t dir, uint32_t frameCounter )
{
    if( size == 0 )
    {
        return LORAMAC_CRYPTO_SUCCESS;  // no payload
    }
    if( buffer == 0 )
    {
    ...
    LoRaMacCommonCopyUint32ToArray( &(aBlock[6]), &address, 4 );
    LoRaMacCommonCopyUint32ToArray( &(aBlock[10]), &frameCounter, 4 );
...
static LoRaMacCryptoStatus_t VerifyCmacB0( uint8_t* msg, uint16_t len, KeyIdentifier_t keyID, bool isAck, uint8_t dir, uint32_t devAddr, uint32_t fCnt, uint32_t expectedCmac )
{
    ...
    uint8_t micBuff[MIC_BLOCK_BX_SIZE];
    memset1( micBuff, 0, MIC_BLOCK_BX_SIZE );

    // Initialize the first Block
    PrepareB0( len, keyID, isAck, dir, devAddr, fCnt, micBuff );

    SecureElementStatus_t retval = SECURE_ELEMENT_ERROR;
    retval = SecureElementVerifyAesCmac( micBuff, msg, len, expectedCmac, keyID );
...
static void UpdateFCntDown( FCntIdentifier_t fCntID, uint32_t currentDown )
{
    switch( fCntID )
    {
        case FCNT_DOWN:
            ...
            LoRaMacSetNvmEvtMibFlag( LORAMAC_NVM_MIBFLG_DOWNLINK_FCNT );
#endif
            break;
#if( USE_LRWAN_1_1_X_CRYPTO == 1 )
        case N_FCNT_DOWN:
...
LoRaMacCryptoStatus_t LoRaMacCryptoInit( LoRaMacCryptoNvmEvent cryptoNvmCtxChanged )
{
    ...
    CryptoCtx.NvmCtx->LrWanVersion.Fields.Patch = 4;
#else
    CryptoCtx.NvmCtx->LrWanVersion.Fields.Patch = 3;
#endif
...
#if ( LORAMAC_USE_RANDOM_DEV_NONCE == 0 )
    CryptoCtx.NvmCtx->DevNonce = DEVNONCE_INITAL_VALUE;
#endif
...
LoRaMacCryptoStatus_t LoRaMacCryptoSetKey( KeyIdentifier_t keyID, uint8_t* key )
{
...
#endif
    {
        ...
        // Derive DataBlockIntKey
        if( LoRaMacCryptoDeriveDataBlockIntKey( keyID ) != LORAMAC_CRYPTO_SUCCESS )
        {
            return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
        }
#endif
...
LoRaMacCryptoStatus_t LoRaMacCryptoPrepareJoinRequest( LoRaMacMessageJoinRequest_t* macMsg )
{
    ...
    // Add device nonce
#if ( LORAMAC_USE_RANDOM_DEV_NONCE == 1 )
    uint32_t devNonce = 0;
    SecureElementRandomNumber( &devNonce );
    CryptoCtx.NvmCtx->DevNonce = devNonce;
#elif  ( LORAMAC_USE_RANDOM_DEV_NONCE == 0 )
    CryptoCtx.NvmCtx->DevNonce++;
#else
#error "LORAMAC_USE_RANDOM_DEV_NONCE value invalid"
#endif
    LoRaMacSetNvmEvtMibFlag( LORAMAC_NVM_MIBFLG_DEV_NONCE );
...
LoRaMacCryptoStatus_t LoRaMacCryptoHandleJoinAccept( JoinReqIdentifier_t joinReqType, uint8_t* joinEUI, LoRaMacMessageJoinAccept_t* macMsg )
{
    ...
    {
        return LORAMAC_CRYPTO_ERROR_INVALID_VERSION;
    }
#endif
...
#else
    SecureElementStatus_t   seRet;
    seRet = SecureElementAesEncrypt( ( macMsg->Buffer + LORAMAC_MHDR_FIELD_SIZE ),
                                     ( macMsg->BufSize - LORAMAC_MHDR_FIELD_SIZE ),
                                     encryptionKeyID,
                                     ( macMsg->Buffer + LORAMAC_MHDR_FIELD_SIZE ) );
    if( seRet != SECURE_ELEMENT_SUCCESS )
    {
        return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
    }
#endif
    ...
    if( CryptoCtx.NvmCtx->LrWanVersion.Fields.Minor == 0 )
    {
        ...
        if( SecureElementVerifyAesCmac( NULL, macMsg->Buffer, ( macMsg->BufSize - LORAMAC_MIC_FIELD_SIZE ), macMsg->MIC, micComputationKeyID ) != SECURE_ELEMENT_SUCCESS )
        {
            return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
        }
    }
    ...
    else
    {
        ...
        if( SecureElementVerifyAesCmac( NULL, procBuffer, ( macMsg->BufSize + micComputationOffset - LORAMAC_MHDR_FIELD_SIZE - LORAMAC_MIC_FIELD_SIZE ), macMsg->MIC, micComputationKeyID ) != SECURE_ELEMENT_SUCCESS )
        {
            return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
        }
    }
...
#if( LORAMAC_USE_JOIN_NONCE_CHECK == 2 )
    // Check if the JoinNonce is greater as the previous one
    if( currentJoinNonce > CryptoCtx.NvmCtx->JoinNonce )
#elif( LORAMAC_USE_JOIN_NONCE_CHECK == 1 )
    // Check if the JoinNonce is different from the previous one
    if( currentJoinNonce != CryptoCtx.NvmCtx->JoinNonce )
#elif( LORAMAC_USE_JOIN_NONCE_CHECK == 0 )
    // Donot check JoinNonce
#else
#error "RAMAC_USE_JOIN_NONCE_CHECK value invalid"
#endif
    {
        CryptoCtx.NvmCtx->JoinNonce = currentJoinNonce;
        LoRaMacSetNvmEvtMibFlag( LORAMAC_NVM_MIBFLG_APP_NONCE );
    }
#if (LORAMAC_VERSION >= LORAWAN_VERSION_1_0_4)
    else
    {
...
#ifdef LORAMAC_ACTMODE_ABP_ENABLED
    LoRaMacSetNvmEvtMibFlag( LORAMAC_NVM_MIBFLG_UPLINK_FCNT |
                             LORAMAC_NVM_MIBFLG_DOWNLINK_FCNT );
#endif
...
LoRaMacCryptoStatus_t LoRaMacCryptoSecureMessage( uint32_t fCntUp, uint8_t txDr, uint8_t txCh, LoRaMacMessageData_t* macMsg )
{
    ...
    if( fCntUp > CryptoCtx.NvmCtx->FCntList.FCntUp )
    {
        p_frmPayload = &( macMsg->Buffer[ macMsg->BufSize - LORAMAC_MIC_FIELD_SIZE - macMsg->FRMPayloadSize ]);
        retval = PayloadEncrypt( p_frmPayload, macMsg->FRMPayloadSize, payloadDecryptionKeyID, macMsg->FHDR.DevAddr, UPLINK, fCntUp );
        if( retval != LORAMAC_CRYPTO_SUCCESS )
        {
            return retval;
        }
...
#ifdef LORAMAC_ACTMODE_ABP_ENABLED
    if( CryptoCtx.NvmCtx->FCntList.FCntUp != fCntUp )
    {
    ...
    // Add the MIC
    p_frmMic = &( macMsg->Buffer[ macMsg->BufSize - LORAMAC_MIC_FIELD_SIZE ] );
    LoRaMacSerializerDataSetMic( p_frmMic, macMsg->MIC );
...
{
    LoRaMacCryptoStatus_t retval;
    KeyIdentifier_t payloadDecryptionKeyID;
    KeyIdentifier_t micComputationKeyID;
    KeyAddr_t* curItem;
...
#if defined(FUOTA_ENABLED) && (FUOTA_VERSION >= FUOTA_VERSION_2_0_0)
LoRaMacCryptoStatus_t LoRaMacCryptoDeriveDataBlockIntKey( KeyIdentifier_t keyID )
{
    // Prevent other keys than GenAppKey for LoRaWAN 1.0.x or AppKey for LoRaWAN 1.1 or later
    if( ( ( keyID == APP_KEY ) && ( CryptoCtx.NvmCtx->LrWanVersion.Fields.Minor == 0 ) ) ||
        ( ( keyID == GEN_APP_KEY ) && ( CryptoCtx.NvmCtx->LrWanVersion.Fields.Minor == 1 ) ) )
    ...
    if( SecureElementDeriveAndStoreKey( CryptoCtx.NvmCtx->LrWanVersion, compBase, keyID, DATA_BLK_INT_KEY ) != SECURE_ELEMENT_SUCCESS )
    {
        return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
    }
...
uint16_t LoRaMacCryptoGetDevNonce( void )
{
    return CryptoCtx.NvmCtx->DevNonce;
}

void LoRaMacCryptoSetDevNonce( uint16_t devNonce )
{
    CryptoCtx.NvmCtx->DevNonce = devNonce;
}

uint32_t LoRaMacCryptoGetJoinNonce( void )
{
    return CryptoCtx.NvmCtx->JoinNonce;
}

void LoRaMacCryptoSetJoinNonce( uint32_t joinNonce )
{
    CryptoCtx.NvmCtx->JoinNonce = joinNonce;
}

uint32_t LoRaMacCryptoGetDownlinkFCnt( void )
{
    return CryptoCtx.NvmCtx->FCntList.FCntDown;
}

void LoRaMacCryptoSetDownlinkFCnt( uint32_t downlinkFCnt )
{
    CryptoCtx.NvmCtx->FCntList.FCntDown = downlinkFCnt;
}

uint32_t LoRaMacCryptoGetUplinkFCnt( void )
{
    return CryptoCtx.NvmCtx->FCntList.FCntUp;
}

void LoRaMacCryptoSetUplinkFCnt( uint32_t uplinkFCnt )
{
    CryptoCtx.NvmCtx->FCntList.FCntUp = uplinkFCnt;
}
