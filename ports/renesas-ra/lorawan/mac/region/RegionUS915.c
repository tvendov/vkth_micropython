...
/* duty cycle enabled */
uint8_t US915_DutyCycleEnabled = US915_DUTY_CYCLE_ENABLED;
...
static int8_t LimitTxPower( int8_t txPower, int8_t maxBandTxPower, int8_t datarate, uint16_t* channelsMask )
{
    ...
    {// Limit tx power to max 26dBm
        txPowerResult = R_MAX( txPower, TX_POWER_2 );
    }
    else
    {
        if( RegionCommonCountChannels( channelsMask, 0, 4 ) < 50 )
        {// Limit tx power to max 21dBm
            txPowerResult = R_MAX( txPower, TX_POWER_5 );
        }
    }
...
PhyParam_t RegionUS915GetPhyParam( GetPhyParams_t* getPhy )
{
    ...
    switch( getPhy->Attribute )
    {
        ...
        case PHY_DUTY_CYCLE:
        {
            phyParam.Value = US915_DutyCycleEnabled;
            break;
        }
...
void RegionUS915SetPhyParam( SetPhyParams_t* setPhy )
{
    switch( setPhy->Attribute )
    {
        case PHY_DUTY_CYCLE:
        {
            if( US915_DutyCycleEnabled != setPhy->param.dcycle_enabled )
            {
                US915_DutyCycleEnabled = setPhy->param.dcycle_enabled;
#if defined(RP_USE_RADIO_CFG_CHECK)
                Radio.SetPib( PIB_RADIO_CFG_CHECK_ENABLE, (uint8_t *)&US915_DutyCycleEnabled );
#endif
            }
            break;
        }
...
void RegionUS915InitDefaults( InitDefaultsParams_t* params )
{
    ...
    switch( params->Type )
    {
        case INIT_TYPE_INIT:
        {
            ...
            // DutyCycle setting
            US915_DutyCycleEnabled = US915_DUTY_CYCLE_ENABLED;

            // (output) default PhyParams
            params->DefaultDutyCycleOn       = US915_DutyCycleEnabled;             // PHY_DUTY_CYCLE
            params->DefaultChannelsTxPower   = US915_DEFAULT_TX_POWER;             // PHY_DEF_TX_POWER
            params->DefaultChannelsDatarate  = US915_DEFAULT_DATARATE;             // PHY_DEF_TX_DR
            params->DefaultMaxRxWindow       = US915_MAX_RX_WINDOW;                // PHY_MAX_RX_WINDOW
            params->DefaultReceiveDelay1     = US915_RECEIVE_DELAY1;               // PHY_RECEIVE_DELAY1
            params->DefaultReceiveDelay2     = US915_RECEIVE_DELAY2;               // PHY_RECEIVE_DELAY2
            params->DefaultJoinAcceptDelay1  = US915_JOIN_ACCEPT_DELAY1;           // PHY_JOIN_ACCEPT_DELAY1
            params->DefaultJoinAcceptDelay2  = US915_JOIN_ACCEPT_DELAY2;           // PHY_JOIN_ACCEPT_DELAY2
            params->DefaultRx1DrOffset       = US915_DEFAULT_RX1_DR_OFFSET;        // PHY_DEF_DR1_OFFSET
            params->DefaultRx2Frequency      = US915_RX_WND_2_FREQ;                // PHY_DEF_RX2_FREQUENCY
            params->DefaultRx2Dr             = US915_RX_WND_2_DR;                  // PHY_DEF_RX2_DR
            params->DefaultUplinkDwellTime   = 0;                                  // PHY_DEF_UPLINK_DWELL_TIME
            params->DefaultDownlinkDwellTime = 0;                                  // PHY_DEF_DOWNLINK_DWELL_TIME
            params->DefaultMaxEirp           = US915_DEFAULT_MAX_ERP + 2.15;       // PHY_DEF_MAX_EIRP
            params->DefaultAntennaGain       = 0;                                  // PHY_DEF_ANTENNA_GAIN
            params->DefaultAdrAckLimit       = US915_ADR_ACK_LIMIT;                // PHY_DEF_ADR_ACK_LIMIT
            params->DefaultAdrAckDelay       = US915_ADR_ACK_DELAY;                // PHY_DEF_ADR_ACK_DELAY
            break;
        }
        ...
        {
            // Bands
            for (int i = 0 ; i < US915_MAX_NB_BANDS ; i++)
            {
...
void RegionUS915ComputeRxWindowParameters( int8_t datarate, uint8_t minRxSymbols, uint32_t rxError, RxConfigParams_t *rxConfigParams )
{
    ...
    rxConfigParams->Datarate = R_MIN( datarate, US915_RX_MAX_DATARATE );
...
uint8_t RegionUS915ApplyDrOffset( uint8_t downlinkDwellTime, int8_t dr, int8_t drOffset )
{
    ...
    // fail-safe : range of drOffset is not checked when JOIN_ACCEPT is received.
    if(drOffset >= 4)
    {
        drOffset = 0;
    }
    datarate = DatarateOffsetsUS915[dr][drOffset];
