 ...
 */
#define REGION_VERSION_1_0_3_x  (0x01000300)
#define REGION_VERSION_2_1_0_2  (0x02010002)    // RP002-1.0.2
#define REGION_VERSION_2_1_0_3  (0x02010003)    // RP002-1.0.3

#if (LORAMAC_VERSION >= LORAWAN_VERSION_1_0_4)
#define REGION_VERSION      REGION_VERSION_2_1_0_3
#else   // LW1.0.3
#define REGION_VERSION      REGION_VERSION_1_0_3_x
#endif
...
typedef enum ePhyAttribute
{
    ...
    PHY_PING_SLOT_NB_CHANNELS,
     /*!
      * RSSI free channel threshold value
      */
     PHY_RSSI_FREE_THRESHOLD,
     /*!
      * Carrier sense time value
      */
     PHY_CARRIER_SENSE_TIME,
}PhyAttribute_t;
...
typedef enum eInitType
{
     ...
     * Initializes bands.
     */
    INIT_TYPE_BANDS_DCYCLE,
#if defined(RP_USE_RADIO_CFG_CHECK)
    /*
     * Initializes radio config.
     */
    INIT_TYPE_RADIO_CFG,
#endif
}InitType_t;
...
typedef struct sInitDefaultsParams
{
    ...
    As923Group_t initAs923Group;

    /*!
     * Get default PhyParam (use in case Type = INIT_TYPE_INIT)
     */
    bool        DefaultDutyCycleOn;
    int8_t      DefaultChannelsTxPower;
    int8_t      DefaultChannelsDatarate;
    uint32_t    DefaultMaxRxWindow;
    uint32_t    DefaultReceiveDelay1;
    uint32_t    DefaultReceiveDelay2;
    uint32_t    DefaultJoinAcceptDelay1;
    uint32_t    DefaultJoinAcceptDelay2;
    uint8_t     DefaultRx1DrOffset;
    uint32_t    DefaultRx2Frequency;
    uint8_t     DefaultRx2Dr;
    uint8_t     DefaultUplinkDwellTime;
    uint8_t     DefaultDownlinkDwellTime;
    float       DefaultMaxEirp;
    float       DefaultAntennaGain;
    uint16_t    DefaultAdrAckLimit;
    uint16_t    DefaultAdrAckDelay;
...
/*!
 * \brief The function sets a value of a specific phy attribute.
 *
 * \param [IN] region LoRaWAN region.
 *
 * \param [IN] setPhy Pointer to the function parameters.
 *
 * \retval Returns a structure containing the PHY parameter.
 */
void RegionSetPhyParam( LoRaMacRegion_t region, SetPhyParams_t* setPhy );
