...
/* duty cycle enabled   */
uint8_t IN865_DutyCycleEnabled = IN865_DUTY_CYCLE_ENABLED;
...
PhyParam_t RegionIN865GetPhyParam( GetPhyParams_t* getPhy )
{
    ...
    switch( getPhy->Attribute )
    {
        ...
        case PHY_DUTY_CYCLE:
        {
            phyParam.Value = IN865_DutyCycleEnabled;
            break;
        }
...
void RegionIN865SetPhyParam( SetPhyParams_t* setPhy )
{
    switch( setPhy->Attribute )
    {
        case PHY_DUTY_CYCLE:
        {
            if( IN865_DutyCycleEnabled != setPhy->param.dcycle_enabled )
            {
                IN865_DutyCycleEnabled = setPhy->param.dcycle_enabled;
#if defined(RP_USE_RADIO_CFG_CHECK)
                Radio.SetPib( PIB_RADIO_CFG_CHECK_ENABLE, (uint8_t *)&IN865_DutyCycleEnabled );
#endif
            }
            break;
        }
...
void RegionIN865InitDefaults( InitDefaultsParams_t* params )
{
    ...
    switch( params->Type )
    {
        case INIT_TYPE_INIT:
            ...
            // DutyCycle setting
            IN865_DutyCycleEnabled = IN865_DUTY_CYCLE_ENABLED;

            // (output) default PhyParams
            params->DefaultDutyCycleOn       = IN865_DutyCycleEnabled;             // PHY_DUTY_CYCLE
            params->DefaultChannelsTxPower   = IN865_DEFAULT_TX_POWER;             // PHY_DEF_TX_POWER
            params->DefaultChannelsDatarate  = IN865_DEFAULT_DATARATE;             // PHY_DEF_TX_DR
            params->DefaultMaxRxWindow       = IN865_MAX_RX_WINDOW;                // PHY_MAX_RX_WINDOW
            params->DefaultReceiveDelay1     = IN865_RECEIVE_DELAY1;               // PHY_RECEIVE_DELAY1
            params->DefaultReceiveDelay2     = IN865_RECEIVE_DELAY2;               // PHY_RECEIVE_DELAY2
            params->DefaultJoinAcceptDelay1  = IN865_JOIN_ACCEPT_DELAY1;           // PHY_JOIN_ACCEPT_DELAY1
            params->DefaultJoinAcceptDelay2  = IN865_JOIN_ACCEPT_DELAY2;           // PHY_JOIN_ACCEPT_DELAY2
            params->DefaultRx1DrOffset       = IN865_DEFAULT_RX1_DR_OFFSET;        // PHY_DEF_DR1_OFFSET
            params->DefaultRx2Frequency      = IN865_RX_WND_2_FREQ;                // PHY_DEF_RX2_FREQUENCY
            params->DefaultRx2Dr             = IN865_RX_WND_2_DR;                  // PHY_DEF_RX2_DR
            params->DefaultUplinkDwellTime   = 0;                                  // PHY_DEF_UPLINK_DWELL_TIME
            params->DefaultDownlinkDwellTime = 0;                                  // PHY_DEF_DOWNLINK_DWELL_TIME
            params->DefaultMaxEirp           = IN865_DEFAULT_MAX_EIRP;             // PHY_DEF_MAX_EIRP
            params->DefaultAntennaGain       = IN865_DEFAULT_ANTENNA_GAIN;         // PHY_DEF_ANTENNA_GAIN
            params->DefaultAdrAckLimit       = IN865_ADR_ACK_LIMIT;                // PHY_DEF_ADR_ACK_LIMIT
            params->DefaultAdrAckDelay       = IN865_ADR_ACK_DELAY;                // PHY_DEF_ADR_ACK_DELAY
...
void RegionIN865ApplyCFList( ApplyCFListParams_t* applyCFList )
{
    ...
    for( chanIdx = IN865_NUMB_DEFAULT_CHANNELS; chanIdx < IN865_MAX_NB_CHANNELS; chanIdx++ )
    {
    ...
    pos = 0;
    for( chanIdx = IN865_NUMB_DEFAULT_CHANNELS; chanIdx < ( IN865_NUMB_CHANNELS_CF_LIST + IN865_NUMB_DEFAULT_CHANNELS ); chanIdx++ )
    {
        // Channel frequency
...
void RegionIN865ComputeRxWindowParameters( int8_t datarate, uint8_t minRxSymbols, uint32_t rxError, RxConfigParams_t *rxConfigParams )
{
    ...
    rxConfigParams->Datarate = R_MIN( datarate, IN865_RX_MAX_DATARATE );
...
uint8_t RegionIN865LinkAdrReq( LinkAdrReqParams_t* linkAdrReq, int8_t* drOut, int8_t* txPowOut, uint8_t* nbRepOut, uint8_t* nbBytesParsed )
{
    ...
    while( bytesProcessed < linkAdrReq->PayloadSize )
    {
        ...
        if( linkAdrParams.ChMaskCtrl != 6 )
        {
            ...
            {
                for( i = 0; i < IN865_MAX_NB_CHANNELS; i++ )
                {
                    if( ( ( chMask & ( 1 << i ) ) != 0 ) &&
...
#if (IN865_TX_MIN_DATARATE > 0)  // Avoid warning; comparing uint8 value with 0 is meaningless
    if( channelAdd->NewChannel->DrRange.Fields.Min < IN865_TX_MIN_DATARATE )
    {
        drInvalid = true;
    }
#endif
    if( ( channelAdd->NewChannel->DrRange.Fields.Min > channelAdd->NewChannel->DrRange.Fields.Max ) ||
        ( channelAdd->NewChannel->DrRange.Fields.Max > IN865_TX_MAX_DATARATE ) )
    {
        drInvalid = true;
    }
...
bool RegionIN865ChannelsRemove( ChannelRemoveParams_t* channelRemove  )
{
    ...
    ret = RegionCommonChanDisable( NvmCtx.ChannelsMask, id, IN865_MAX_NB_CHANNELS );
    if( ret == true )
    {
...
uint8_t RegionIN865ApplyDrOffset( uint8_t downlinkDwellTime, int8_t dr, int8_t drOffset )
{
    ...
    return R_MIN( DR_5, R_MAX( DR_0, dr - EffectiveRx1DrOffsetIN865[drOffset] ) );
