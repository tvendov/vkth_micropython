...
/* duty cycle enabled   */
uint8_t EU868_DutyCycleEnabled = EU868_DUTY_CYCLE_ENABLED;
...
PhyParam_t RegionEU868GetPhyParam( GetPhyParams_t* getPhy )
{
    ...
    switch( getPhy->Attribute )
    {
        ...
        {
            /* duty cycle enabled   */
            phyParam.Value = EU868_DutyCycleEnabled;
            break;
        }
...
void RegionEU868SetPhyParam( SetPhyParams_t* setPhy )
{
    switch( setPhy->Attribute )
    {
        case PHY_DUTY_CYCLE:
        {
            if( EU868_DutyCycleEnabled != setPhy->param.dcycle_enabled )
            {
                EU868_DutyCycleEnabled = setPhy->param.dcycle_enabled;
#if defined(RP_USE_RADIO_CFG_CHECK)
                Radio.SetPib( PIB_RADIO_CFG_CHECK_ENABLE, (uint8_t *)&EU868_DutyCycleEnabled );
#endif
            }
            break;
        }
...
void RegionEU868InitDefaults( InitDefaultsParams_t* params )
{
    ...
    switch( params->Type )
    {
        case INIT_TYPE_INIT:
            ...
            // DutyCycle setting
            EU868_DutyCycleEnabled = EU868_DUTY_CYCLE_ENABLED;

            // (output) default PhyParams
            params->DefaultDutyCycleOn       = EU868_DutyCycleEnabled;             // PHY_DUTY_CYCLE
            params->DefaultChannelsTxPower   = EU868_DEFAULT_TX_POWER;             // PHY_DEF_TX_POWER
            params->DefaultChannelsDatarate  = EU868_DEFAULT_DATARATE;             // PHY_DEF_TX_DR
            params->DefaultMaxRxWindow       = EU868_MAX_RX_WINDOW;                // PHY_MAX_RX_WINDOW
            params->DefaultReceiveDelay1     = EU868_RECEIVE_DELAY1;               // PHY_RECEIVE_DELAY1
            params->DefaultReceiveDelay2     = EU868_RECEIVE_DELAY2;               // PHY_RECEIVE_DELAY2
            params->DefaultJoinAcceptDelay1  = EU868_JOIN_ACCEPT_DELAY1;           // PHY_JOIN_ACCEPT_DELAY1
            params->DefaultJoinAcceptDelay2  = EU868_JOIN_ACCEPT_DELAY2;           // PHY_JOIN_ACCEPT_DELAY2
            params->DefaultRx1DrOffset       = EU868_DEFAULT_RX1_DR_OFFSET;        // PHY_DEF_DR1_OFFSET
            params->DefaultRx2Frequency      = EU868_RX_WND_2_FREQ;                // PHY_DEF_RX2_FREQUENCY
            params->DefaultRx2Dr             = EU868_RX_WND_2_DR;                  // PHY_DEF_RX2_DR
            params->DefaultUplinkDwellTime   = 0;                                  // PHY_DEF_UPLINK_DWELL_TIME
            params->DefaultDownlinkDwellTime = 0;                                  // PHY_DEF_DOWNLINK_DWELL_TIME
            params->DefaultMaxEirp           = EU868_DEFAULT_MAX_EIRP;             // PHY_DEF_MAX_EIRP
            params->DefaultAntennaGain       = EU868_DEFAULT_ANTENNA_GAIN;         // PHY_DEF_ANTENNA_GAIN
            params->DefaultAdrAckLimit       = EU868_ADR_ACK_LIMIT;                // PHY_DEF_ADR_ACK_LIMIT
            params->DefaultAdrAckDelay       = EU868_ADR_ACK_DELAY;                // PHY_DEF_ADR_ACK_DELAY
        ...
        {
            // Bands
            for (int i = 0 ; i < EU868_MAX_NB_BANDS ; i++)
            {
...
void RegionEU868ApplyCFList( ApplyCFListParams_t* applyCFList )
{
    ...
    for( chanIdx = EU868_NUMB_DEFAULT_CHANNELS; chanIdx < EU868_MAX_NB_CHANNELS; chanIdx++ )
    {
    ...
    pos = 0;
    for( chanIdx = EU868_NUMB_DEFAULT_CHANNELS; chanIdx < ( EU868_NUMB_CHANNELS_CF_LIST + EU868_NUMB_DEFAULT_CHANNELS ); chanIdx++ )
    {
        // Channel frequency
...
void RegionEU868ComputeRxWindowParameters( int8_t datarate, uint8_t minRxSymbols, uint32_t rxError, RxConfigParams_t *rxConfigParams )
{
    ...
    rxConfigParams->Datarate = R_MIN( datarate, EU868_RX_MAX_DATARATE );
...
#if (EU868_TX_MIN_DATARATE > 0)  // Avoid warning; comparing uint8 value with 0 is meaningless
    if( channelAdd->NewChannel->DrRange.Fields.Min < EU868_TX_MIN_DATARATE )
    {
        drInvalid = true;
    }
#endif
    if( ( channelAdd->NewChannel->DrRange.Fields.Min > channelAdd->NewChannel->DrRange.Fields.Max ) ||
        ( channelAdd->NewChannel->DrRange.Fields.Max > EU868_TX_MAX_DATARATE ) )
    {
        drInvalid = true;
    }
...
bool RegionEU868ChannelsRemove( ChannelRemoveParams_t* channelRemove  )
{
    ...
    ret = RegionCommonChanDisable( NvmCtx.ChannelsMask, id, EU868_MAX_NB_CHANNELS );
    if( ret == true )
    {
