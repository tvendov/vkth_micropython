...
/*!
 * Holds the desired LoRaWAN network type and the last register image.
 */
typedef struct
{
    bool Desired;
...
static RadioPublicNetwork_t RadioPublicNetwork = { false, false };

static void RadioApplyLoRaSyncWord( void )
{
    uint16_t syncWord = RadioPublicNetwork.Desired ?
        LORA_MAC_PUBLIC_SYNCWORD : LORA_MAC_PRIVATE_SYNCWORD;

    SX126xWriteRegister( REG_LR_SYNCWORD, ( syncWord >> 8 ) & 0xFF );
    SX126xWriteRegister( REG_LR_SYNCWORD + 1, syncWord & 0xFF );
    RadioPublicNetwork.Current = RadioPublicNetwork.Desired;
}

static void RadioApplyLoRaSyncWordFromMib( void )
{
    ...
    {
        RadioPublicNetwork.Desired = mib.Param.EnablePublicNetwork;
    }
    RadioApplyLoRaSyncWord( );
}
...
RadioResult_t RadioInit( RadioEvents_t *events )
{
    ...
    RadioPublicNetwork.Desired = false;
                         ...
                         bool iqInverted, bool rxContinuous )
{
    ...
    switch( modem )
    {
        ...
        case MODEM_LORA:
            ...
            SX126xSetModulationParams( &SX126x.ModulationParams );
            SX126xSetPacketParams( &SX126x.PacketParams );
            /* RX-window wakeup may be touched by PrivateLoRa radio code before
             * this point. Re-read the LoRaMac MIB and assert the LoRaWAN
             * public/private syncword required by the active MAC context. */
            RadioApplyLoRaSyncWordFromMib( );
...
void RadioSetPublicNetwork( bool enable )
{
    RadioPublicNetwork.Desired = enable;
    RadioSetModem( MODEM_LORA );
}
