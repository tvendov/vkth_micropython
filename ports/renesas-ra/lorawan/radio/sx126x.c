 ...
 */
static RadioOperatingModes_t OperatingMode = MODE_STDBY_RC;
...
volatile uint32_t CurrentFrequency = 0;
...
#if defined (RP_USE_RF_SWITCH)
    SX126xAntSwOn();
    SX126xSetDio2AsRfSwitchCtrl( true );
#else //#if defined (RP_USE_RF_SWITCH)
    SX126xSetDio2AsRfSwitchCtrl( false );
#endif //#if defined (RP_USE_RF_SWITCH)

    SX126xSetOperatingMode( MODE_STDBY_RC );

    PacketType = PACKET_TYPE_GFSK; // initial packet type is GFSK
...
void SX126xSetSleep( SleepParams_t sleepConfig )
{
    ...
    // Chipmode 0x2:STBY_RC, 0x3:STBY_XOSC
    if( (status.Fields.ChipMode != 0x2)
     && (status.Fields.ChipMode != 0x3) )
    {
        SX126xSetStandby( STDBY_RC );
    }
    ...
    if(sleepConfig.Fields.WarmStart == 1)
    {
        SX126xSetOperatingMode( MODE_SLEEP );
    }
    else
    {
        SX126xSetOperatingMode( MODE_COLD_SLEEP );
        ImageCalibrated = false;
        CurrentFrequency = 0;
        PacketType = PACKET_TYPE_GFSK; // initial packet type is GFSK
    }
...
void SX126xSetStandby( RadioStandbyModes_t standbyConfig )
{
    ...
    {
        SX126xSetOperatingMode( MODE_STDBY_RC );
        SX126xSetRxTxFallbackMode( FB_STDBY_RC );
    }
    else
    {
...
void SX126xSetRxTxFallbackMode( RadioFallbackModes_t fallbackMode )
{
    SX126xWriteCommand( RADIO_SET_TXFALLBACKMODE, (uint8_t *)&fallbackMode, 1 );
}
...
int16_t SX126xGetRssiInst( void )
{
    uint8_t buf[1];
    int16_t rssi = 0;
    ...
    rssi = - (buf[0] >> 1);
    return SX126xCalculateOffsettedRssi(rssi);
}
...
void SX126xGetPacketStatus( PacketStatus_t *pktStatus )
{
    ...
    switch( pktStatus->packetType )
    {
        case PACKET_TYPE_GFSK:
            ...
            pktStatus->Params.Gfsk.RssiSync = SX126xCalculateOffsettedRssi(-(status[1] >> 1));
            pktStatus->Params.Gfsk.RssiAvg  = SX126xCalculateOffsettedRssi( -(status[2] >> 1));
            pktStatus->Params.Gfsk.FreqError = 0;
            break;

        case PACKET_TYPE_LORA:
            pktStatus->Params.LoRa.RssiPkt =SX126xCalculateOffsettedRssi( -(status[0] >> 1) );
            // Returns SNR value [dB] rounded to the nearest integer value
            pktStatus->Params.LoRa.SnrPkt = ( ( ( int8_t )status[1] ) + 2 ) >> 2;
            pktStatus->Params.LoRa.SignalRssiPkt =SX126xCalculateOffsettedRssi( -(status[2] >> 1) );
...
void SX126xSetXtalTrim( void )
{
    ...
    {
        RadioStandbyModes_t mode = STDBY_XOSC;
        SX126xWriteCommand( RADIO_SET_STANDBY, ( uint8_t* )&mode, 1 );
        Radio.Write(REG_XTA_TRIM, PibValues.xtalXtaTrim);
        Radio.Write(REG_XTB_TRIM, PibValues.xtalXtbTrim);
    }
}

void SX126xRecoverBoardConfig( void )
{
#if defined (RP_USE_RF_SWITCH)
    SX126xSetDio2AsRfSwitchCtrl( true );
#else //#if defined (RP_USE_RF_SWITCH)
...
int16_t SX126xCalculateOffsettedRssi(int16_t rssi)
{
    rssi += PibValues.rssiOffset;

    return ((rssi <= INT8_MIN) ? INT8_MIN : rssi);
}

uint32_t SX126xGetRfFrequency(void)
{
