...
void RadioSetModem( RadioModems_t modem )
{
    ...
    case MODEM_LORA:
        ...
        // PATCH: SetPacketType resets the LoRa sync word register on
        // the chip. The original `if Current != Previous` guard skips
        // the re-apply whenever Current==Previous (which is the normal
        // case after SetPublicNetwork), leaving the chip with the
        // default private sync word 0x14/0x24 instead of the LoRaWAN
        // public 0x34/0x44 — silently breaking all downlink RX.
                         ...
                         bool iqInverted, bool rxContinuous )
{
    ...
    switch( modem )
    {
        ...
        case MODEM_LORA:
            ...
            // PATCH: belt-and-suspenders sync-word re-apply after all
            // packet/modulation params are written. Without this we
            // observed (via mac.radio_read_reg(0x0740)) that the LoRa
            // sync register reverts to the chip's private 0x14/0x24
            // default after SetPacketParams, even though earlier
            // RadioSetPublicNetwork wrote 0x34/0x44 — likely some
            // intermediate opcode resets it. Writing here guarantees
            // the chip is in LoRaWAN public sync mode immediately
            // before SetRx is called by the caller.
            if( RadioPublicNetwork.Current == true )
            {
                SX126xWriteRegister( REG_LR_SYNCWORD,
                    ( LORA_MAC_PUBLIC_SYNCWORD >> 8 ) & 0xFF );
